...
use crate::{textfile, SnvId, SumStat};

//use io_rust::{text, Snv};

// TODO: variable col
pub fn load_loss(fin_loss: &Path) -> Vec<SumStat> {
    textfile::check_open_file(fin_loss);

    let m = textfile::compute_num_line_text(fin_loss, None).unwrap();
    ...
    let vss: Vec<Vec<String>> = textfile::load_table_cols(&fin_loss, &cols, true);

...
pub fn load_vars(
    ...
    let cols_name = textfile::load_table_header(fin_var.unwrap());

    let valss = textfile::load_table(fin_var.unwrap(), true);

    // TODO: fin_var do not have to be the same as n_in
    // fin_var should be ok only when it contains all samples
...
pub fn vals_align_id(
    ...
    let v = sample::vals_align_id(vals, ids, sample_id_to_n);

    let vals_align: Vec<f64> = v.into_iter().map(|x| x.parse::<f64>().unwrap()).collect();

    vals_align

    /*     //vals_str.iter().for_each(|x| {println!("{}",x); x.parse::<f64>().unwrap();});

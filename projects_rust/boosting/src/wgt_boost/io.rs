...
use crate::BoostType;
use genetics::{textfile, vec, SnvId};
...
pub fn get_file_acc(dout_para: &Path) -> PathBuf {
    let mut d = dout_para.to_owned();
    d.push("acc.tsv");
    d
}
...
pub fn load_wgts_file(fwgt: &Path) -> Vec<WgtBoost> {
    ...
    // TODO: make model and coef match one-by-one
    let boost_coef_type = load_wgts_boost_coef_type(fwgt);

    log::debug!("Boost coef type {:?}", boost_coef_type);
...
mod tests {
    ...
    fn test_get_dname_para() {
        ...
        assert_eq!(dout_para, PathBuf::from("./abc/para_lr-0.1/"));
    }

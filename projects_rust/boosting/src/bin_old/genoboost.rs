...
fn get_matches() -> ArgMatches<'static> {
    let matches = app_from_crate!()
        ...
        .subcommand( SubCommand::with_name("train").about("Train model.")
            ...
            .arg(Arg::from_usage("--batch [BATCH] 'When to recreate batch.'").default_value("fix_50_10_50"))
            ...
            .arg(Arg::from_usage("--iter_snv [NUM] 'Number of SNVs for iterations.'").default_value("10000"))
            ...
            .arg(
                Arg::from_usage("--boost_type [BOOSTTYPE] 'Boosting method to use.'")
                    .possible_values(&["nonadd", "add"])
                    .default_value("nonadd"),
            )
            .arg(Arg::from_usage("--file_sample [FILE] 'File of samples for training.'"))
            //.arg(Arg::from_usage("--file_sample_val [FILE] 'File of samples for validation.'"))
            .arg(Arg::from_usage("--file_phe [FILE] 'File of phenotype to use. If not set, phenotype in [--file_plink] is used.'"))
            // TODO
            ...
            .arg(Arg::from_usage("--learning_rates [VALS] 'Learning rates.'").default_value("0.5 0.2 0.1 0.05").multiple(true))
            .arg(Arg::from_usage("--eff_eps [VAL] 'How to adjust s2.'").default_value("lims2gmodeloverkeepsignprop_100_4_4_0.8"))
        )
        .subcommand( SubCommand::with_name("score").about("Calculate score.")
            ...
            .arg(Arg::from_usage("--file_cov [FILE] 'File of covariates to use.'"))
            .arg(Arg::from_usage("--dir_wgt [FILE] 'Directory of a weight file.'"))
            ...
            .arg(Arg::from_usage("--learning_rates [VALS] 'Learning rates. Necessary when use --dir_wgt'").default_value("0.5 0.2 0.1 0.05").multiple(true))
...
fn main() {
    ...
    if let Some(ref matches) = matches.subcommand_matches("train") {
        ...
        let boost_param = boost_param.set_iteration_snv(
            matches
                .value_of("iter_snv")
                .unwrap()
                .parse::<usize>()

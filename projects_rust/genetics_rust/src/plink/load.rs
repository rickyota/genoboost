...
fn load_buf_size_max(fin: &Path) -> usize {
    let mut buf_size_max = 0usize;
    for chrom_i in Chrom::variants().iter() {
        let file_size = text::file_size(&plink::fname_bed(&fin, Some(chrom_i)));
...
pub fn generate_genot(
    ...
    if is_split_chrom {
        ...
        let buf_size_max = load_buf_size_max(fin).min(buf_size_limit);
        ...
        for chrom_i in Chrom::variants().iter() {
            ...
            let reader = BufReader::with_capacity(
                reader_cap,
                File::open(plink::fname_bed(&fin, Some(chrom_i))).unwrap(),
            );
...
mod tests {
    ...
    fn setup_test() -> (PathBuf, Vec<bool>, usize, usize, Vec<bool>, Vec<bool>) {
        ...
        let ys: Vec<bool> = plink::load_ys(&fin, None,None,n, &use_samples);

...
pub fn load_genotypes_for_score<W: WgtTrait>(
    ...
    let (use_snvs, is_reversed) = create_wgt_to_genotype_index_sametime(fin, wgts);
...
fn check_consistent_is_reversed(is_reversed: &[bool],
...
pub fn load_genotypes_for_score_multiwgts(
    ...
    for wgts in wgts_multi.iter_mut() {
        // length are m_in
        let use_snvs = create_wgt_to_genotype_index(fin, wgts.wgts());
...
///
fn create_wgt_to_genotype_index<W: WgtTrait>(fin: &Path, wgts: &[W]) -> Vec<bool> {
    let m_in: usize = plink::compute_num_snv(fin).unwrap();
    let snvs_in = plink::load_snvs(fin, m_in);

...
//pub fn load_table_col_by_header(fin: &Path, col: &str) -> Option<Vec<String>> {
//    let cols=vec![col];
//    load_table_cols_by_header(fin, &cols)
//}


/// assume no duplicated header
pub fn load_table_cols_by_header(fin: &Path, cols: &[&str]) -> Option<Vec<Vec<String>>> {
    let header=load_table_header(fin).unwrap();

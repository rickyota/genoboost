...
impl BoostParam {
    pub fn check(self) {
        ...
        //if self.boost_type().is_logit() {
        //    if self.eps() != Eps::MedLarge2 {
        //        panic!("Cannot assign except MedLarge2 in Logit")
        //    }
        //}

/*         if self.boost_type().is_type_logit() & self.eps().allcell() {
            panic!("Eps allcell is not ideal since this affects score of counts>>0")
        } */

        //if self.boost_type().is_type_logit() & !self.eps().none() {
        //    panic!("Cannot assign except Eps::None in Logit")
        //}

        //if self.boost_type().is_type_logit() & !self.sample_weight_clip().is_none() {
        //    panic!("Cannot assign except SampleWeightClip::None in Logit")
        //}
...
/*     // TO delete
    pub fn new_str(
        iteration: IterationNumber, // TODO: from usize or string
        boost_type: &str,
        loss_func: &str,
        learning_rate: Option<f64>,
        eps: Option<&str>,
        sample_weight_clip: Option<&str>,
        sample_weight_wls_clip: Option<&str>,
        is_dom_rec: bool,
        cov_way: Option<&str>,
        batch_way: Option<&str>,
        eff_eps: Option<&str>,
    ) -> Self {
        let bp = BoostParam {
            iteration,
 ...
 */
    ...
    // do not check in set_*()
    // later run check() above
    ...
    pub fn set_learning_rate(self, learning_rate: Option<f64>) -> Self {
        BoostParam {
            learning_rate,
            ..self
        }
...
impl Default for BoostType{
    fn default() -> Self{Self::LogitNoMissing}
}
...
impl BatchWay {
    ...
    pub fn batch_max_iter(self, learning_rate: Option<f64>) -> usize {
        match self {
            Self::Every(x) => x,
            Self::EveryMaxIter(_, x1, x2) | Self::Fix(_, x1, x2) => match learning_rate {
                Some(lr) => {
                    if lr > 0.6f64 {
                        x1
                    } else {

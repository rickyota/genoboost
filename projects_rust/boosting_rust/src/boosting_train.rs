...
pub fn boosting_covs(
    ...
    let wgtcovs_logreg = regression_cov::logistic_regression_covs(
        dataset.samples(),
        iteration_start,
    );
...
fn run_cov_iteration(wgts: &WgtBoosts, cov_way: Option<CovWay>) -> bool {
    match cov_way {
        ...
        Some(cov_way) => match cov_way {
            ...
            CovWay::Every(x) => {
                panic!("Cannot run weighted regression. implement to allow cov_way=first only");
                if wgts.wgts_n() == 0 {
                    return true;
                }
...
pub fn boosting_batch<W: std::io::Write>(
    ...
    while run_next_iteration(ti, iteration, &wgts) {
        ...
        ) & run_next_iteration(ti, iteration, &wgts)
        {
            ...
            if let Some(dataset_val) = dataset_val {
                let nsnvs = wgts.count_unique_snv();
                if nsnvs_monitor.contains(&nsnvs) {
                    ...
                    let is_exit = monitor_acc(&mut acc_monitor, &nsnvs_monitor, nsnvs);
                    if is_exit {
...
fn update_acc_monitor(
    ...
    let nsnvs_index = nsnvs_monitor.iter().position(|x| *x == nsnvs).unwrap();
    acc_monitor[nsnvs_index] = acc
}

fn monitor_acc(acc_monitor: &mut [f64], nsnvs_monitor: &[usize], nsnvs: usize) -> bool {
    ...
    let nsnvs_max = nsnvs_monitor[index_max];
    log::debug!("nsnvs_max {}", nsnvs_max);
    log::debug!("nsnvs_current {}", nsnvs);

    if nsnvs_max < 1000 {
        return false;
    }
    //if nsnvs > nsnvs_max+2000 {
    if (nsnvs as f64) > (nsnvs_max as f64) * 1.5f64 {
        return true;
    }

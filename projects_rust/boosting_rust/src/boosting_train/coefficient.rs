...
pub fn coef_lr(coef: Coef, learning_rate: Option<f64>, boost_type: BoostType) -> Coef {
    let lr = learning_rate.unwrap_or(1.0);
...
mod tests {
    ...
    fn test_calculate_coefficients_freemodelmissing() {
        ...
        let coef = calculate_coefficients_ada(t, BoostType::FreeModelMissing, None, Some(Eps::Med));
        assert_eq!(
            coef,
    ...
    fn test_calculate_coefficients_freemodelmissing_lr() {
        let t = ContingencyTable::new_seven((0.02, 0.01, 0.1, 0.2, 0.3, 0.3, 0.07));
        let coef =
            calculate_coefficients_ada(t, BoostType::FreeModelMissing, Some(0.1), Some(Eps::Med));
        assert_eq!(
            coef,

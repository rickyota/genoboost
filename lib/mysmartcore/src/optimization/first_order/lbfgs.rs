...
pub struct LBFGS {
    ...
    ///
    pub g_atol: f64,
    // below should be 0.0, equvalent to no effects
    ///
    pub x_atol: f64,
    ///
    pub x_rtol: f64,
...
// added by R. Ohta
fn clip_s<T: FloatNumber + RealNumber>(x: T) -> T {
    let thre = 10.0f64;
    if x > T::from(thre).unwrap() {
        T::from(thre).unwrap()
    } else if x < T::from(-thre).unwrap() {
        T::from(-thre).unwrap()
    } else {
        x
    }
...
impl LBFGS {
    ...
    fn two_loops<T: FloatNumber + RealNumber, X: Array1<T>>(&self, state: &mut LBFGSState<T, X>) {
        ...
        //println!("1 state.twoloop_q {:?}",state.twoloop_q);
        state.twoloop_q.copy_from(&state.x_df);
        //println!("2 state.twoloop_q {:?}",state.twoloop_q);
        ...
        for index in (lower..upper).rev() {
            ...
            //println!("3 state.twoloop_q {:?}",state.twoloop_q);
        }
        ...
        if state.iteration > 0 {
            ...
            //println!("1 state.s {:?}",state.s);
        } else {
            state.s.copy_from(&state.twoloop_q);
            //println!("2 state.s {:?}",state.s);

            // added by R.Ohta
            // cilp initial coef
            // this successfully avoided ouput nan
            state.s.iterator_mut(0).for_each(|x| *x = clip_s(*x));
        }
    ...
    fn update_state<'a, T: FloatNumber + RealNumber, X: Array1<T>, LS: LineSearchMethod<T>>(
        ...
        println!("state.x_prev {:?}",state.x_prev);
        println!("state.x_f {:?}",state.x_f);
        println!("state.x_f_prev {:?}",state.x_f_prev);
        println!("state.x_df {:?}",state.x_df);
        println!("state.x_df_prev {:?}",state.x_df_prev);
        println!("state.s {:?}",state.s);  */
        ...
        //state.alpha=T::from(1e-3).unwrap();
        //state.alpha=T::from(1e-8).unwrap();
        //state.alpha=T::from(1e-4).unwrap();
        //state.alpha=T::from(10.0).unwrap();

        /*         println!("\naaa");
        println!("state.alpha {:?}",state.alpha);
        println!("state.s {:?}",state.s);
        println!("state.x {:?}",state.x); */
    ...
    fn assess_convergence<T: FloatNumber, X: Array1<T>>(
        ...
        /*         println!("x: {:?}", state.x);
        println!("x_prev: {:?}", state.x_prev);

        println!(
            "max_diff {}, {}",
            state.x.max_diff(&state.x_prev),
            self.x_rtol * state.x.norm(std::f64::INFINITY)
        ); */
...
impl<T: FloatNumber + RealNumber> FirstOrderOptimizer<T> for LBFGS {
    ...
    fn optimize<'a, X: Array1<T>, LS: LineSearchMethod<T>>(
        ...
        /*         // modified by R. Ohta
        println!("max-iter {}", self.max_iter);
        println!("gtol {}", self.g_atol); */

        let mut state = self.init_state(x0);

        //println!("init state.x {:?}", state.x);

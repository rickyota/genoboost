...
static inline PgenReaderMain *GetPgrp(PgenReader *pgr_ptr) { return &GET_PRIVATE(*pgr_ptr, m); }

static inline const uint32_t *GetSicp(PgrSampleSubsetIndex pssi) { return GET_PRIVATE(pssi, cumulative_popcounts); }
...
#endif
void GenoarrbCountSubsetFreqs(const unsigned char *genoarrb, const uintptr_t *__restrict sample_include_interleaved_vec, uint32_t raw_sample_ct,
                              uint32_t sample_ct, STD_ARRAY_REF(uint32_t, 4) genocounts) {
    // does not read past the end of genoarrb
    ...
    CountSubset3FreqVec6(R_CAST(const VecW *, genoarrb), R_CAST(const VecW *, sample_include_interleaved_vec), vec_idx, &even_ct, &odd_ct,
                         &bothset_ct);
...
PglErr PgfiInitPhase1(const char *fname, uint32_t raw_variant_ct, uint32_t raw_sample_ct, uint32_t use_mmap, PgenHeaderCtrl *header_ctrl_ptr,
                      PgenFileInfo *pgfip, uintptr_t *pgfi_alloc_cacheline_ct_ptr, char *errstr_buf) {
    ...
    if(unlikely(use_mmap)) {
        ...
        snprintf(errstr_buf, kPglErrstrBufBlen,
                 "Error: PgfiInitPhase1() use_mmap parameter is nonzero, but pgenlib was not compiled with mmap support.\n");
    ...
    *header_ctrl_ptr = 0;
    if(file_type_code < 2) {
        ...
        if(unlikely(!file_type_code)) {
            ...
            snprintf(errstr_buf, kPglErrstrBufBlen,
                     "Error: pgenlib does not directly support sample-major PLINK 1 .bed files.\n(However, PLINK 2 automatically transposes and "
        ...
        if(unlikely(raw_sample_ct == UINT32_MAX)) {
            ...
            snprintf(errstr_buf, kPglErrstrBufBlen,
                     "Error: PgfiInitPhase1() must be called with an accurate raw_sample_ct value, since %s is a PLINK 1 .bed file.\n", fname);
            return kPglRetImproperFunctionCall;
        }
        ...
        if(raw_variant_ct == UINT32_MAX) {
            ...
            if(unlikely((quotient > 0x7fffffffU) || (quotient * const_vrec_width + 3 != fsize))) {
                snprintf(errstr_buf, kPglErrstrBufBlen,
                         "Error: Unexpected PLINK 1 .bed file size (since raw_sample_ct was %u, [file size - 3] should be divisible by %u and the "
                         "quotient should be smaller than 2^31).\n",
    ...
    if(file_type_code < 16) {
        ...
        if(unlikely(file_type_code > 4)) {
            snprintf(errstr_buf, kPglErrstrBufBlen,
                     "Error: Third byte of %s does not correspond to a storage mode supported by this version of pgenlib.\n", fname);
            return kPglRetNotYetSupported;
        }
        if(unlikely(header_ctrl & 63)) {
            snprintf(errstr_buf, kPglErrstrBufBlen,
                     "Error: Third byte of %s corresponds to a fixed-width storage mode, but twelfth byte is only consistent with a variable-width "
                     "mode.\n",
                     fname);
            return kPglRetMalformedInput;
        }
    ...
    if(header_ctrl & 8) {
        ...
        if(!header_ctrl_low3) {
            ...
            snprintf(errstr_buf, kPglErrstrBufBlen,
                     "Error: Twelfth byte of %s does not correspond to a format supported by this version of pgenlib.\n", fname);
            return kPglRetNotYetSupported;
        }
    } else {
...
PglErr PgfiInitPhase2(PgenHeaderCtrl header_ctrl, uint32_t allele_cts_already_loaded, uint32_t nonref_flags_already_loaded, uint32_t use_blockload,
                      uint32_t vblock_idx_start, uint32_t vidx_end, uint32_t *max_vrec_width_ptr, PgenFileInfo *pgfip, unsigned char *pgfi_alloc,
                      uintptr_t *pgr_alloc_cacheline_ct_ptr, char *errstr_buf) {
        ...
        *max_vrec_width_ptr = const_vrec_width;
        ...
        if(!shared_ff) {
            if(unlikely(use_blockload)) {
                snprintf(errstr_buf, kPglErrstrBufBlen,
                         "Error: PgfiInitPhase2() cannot be called with use_blockload set when PgfiInitPhase1() had use_mmap set.\n");
                return kPglRetImproperFunctionCall;
            }
...
for(;; ++vblock_idx) {
    if(vblock_idx >= vblock_ct_m1) {
        if(vblock_idx > vblock_ct_m1) {
            ...
            *pgr_alloc_cacheline_ct_ptr =
                CountPgrAllocCachelinesRequired(raw_sample_ct, new_gflags, max_allele_ct, (shared_ff && (!use_blockload)) ? max_vrec_width : 0);
...
PglErr PgfiMultiread(const uintptr_t *variant_include, uint32_t variant_uidx_start, uint32_t variant_uidx_end, uint32_t load_variant_ct,
                     PgenFileInfo *pgfip) {
...
PglErr ParseDifflistHeader(const unsigned char *fread_end, uint32_t raw_sample_ct, const unsigned char **fread_pp, uintptr_t *raregeno_buf,
                           const unsigned char **difflist_group_info_ptr, uint32_t *difflist_len_ptr) {
...
PglErr ParseAndSaveDifflist(const unsigned char *fread_end, uint32_t raw_sample_ct, const unsigned char **fread_pp, uintptr_t *__restrict raregeno,
                            uint32_t *__restrict difflist_sample_ids, uint32_t *__restrict difflist_len_ptr) {
...
PglErr ParseAndSaveDifflistProperSubset(const unsigned char *fread_end, const uintptr_t *__restrict sample_include,
                                        const uint32_t *__restrict sample_include_cumulative_popcounts, uint32_t raw_sample_ct,
                                        const unsigned char **fread_pp, uintptr_t *__restrict raregeno, uint32_t *__restrict difflist_sample_ids,
                                        uint32_t *__restrict difflist_len_ptr, uintptr_t *__restrict raregeno_workspace) {
    ...
    for(uint32_t subgroup_idx = 0;; ++subgroup_idx) {
        ...
        for(uint32_t raw_difflist_idx_lowbits = 0;; ++raw_difflist_idx_lowbits) {
            ...
            if(IsSet(sample_include, raw_sample_idx)) {
                ...
                difflist_sample_ids_iter[difflist_len_lowbits] =
                    RawToSubsettedPos(sample_include, sample_include_cumulative_popcounts, raw_sample_idx);
...
PglErr ParseLdAndMergeDifflistSubset(const unsigned char *fread_end, const uintptr_t *__restrict sample_include,
                                     const uint32_t *__restrict sample_include_cumulative_popcounts, const uintptr_t *__restrict ldbase_raregeno,
                                     const uint32_t *__restrict ldbase_difflist_sample_ids, uint32_t ldbase_difflist_len,
                                     uintptr_t ldbase_common_geno, uint32_t raw_sample_ct, uint32_t sample_ct, const unsigned char **fread_pp,
                                     uintptr_t *__restrict merged_raregeno, uint32_t *__restrict merged_difflist_sample_ids,
...
/*
void PrunedDifflistToGenovecSubsetUnsafe(const uintptr_t* __restrict sample_include, const uint32_t* __restrict sample_include_cumulative_popcounts,
const uintptr_t* __restrict raregeno, const uint32_t* __restrict difflist_sample_ids, uint32_t sample_ct, uint32_t difflist_common_geno, uint32_t
difflist_len, uintptr_t* __restrict genovec) {
...
PglErr ParseAndApplyDifflistSubset(const unsigned char *fread_end, const uintptr_t *__restrict sample_include,
                                   const uint32_t *__restrict sample_include_cumulative_popcounts, uint32_t sample_ct, const unsigned char **fread_pp,
                                   PgenReaderMain *pgrp, uintptr_t *__restrict genoarr) {
    // Side effects: uses pgr.workspace_raregeno_tmp_loadbuf.
    const uint32_t raw_sample_ct = pgrp->fi.raw_sample_ct;
...
PglErr ParseOnebitUnsafe(const unsigned char *fread_end, const unsigned char **fread_pp, PgenReaderMain *pgrp, uintptr_t *__restrict genoarr) {
    ...
    if(read_hw_ct >= 2 * kWordsPerVec) {
        ...
        // are 0 and 1
        const VecW lookup = {word_base + common_code_delta * 0x1514111005040100LLU, word_base + common_code_delta * 0x5554515045444140LLU};
#else
        const VecW m1 = VCONST_W(kMask5555);
...
// vector-alignment preferred
PglErr Parse1or2bitGenoarrUnsafe(const unsigned char *fread_end, uint32_t vrtype, const unsigned char **fread_pp, PgenReaderMain *pgrp,
...
PglErr ParseNonLdGenovecSubsetUnsafe(const unsigned char *fread_end, const uintptr_t *__restrict sample_include,
                                     const uint32_t *__restrict sample_include_cumulative_popcounts, uint32_t sample_ct, uint32_t vrtype,
                                     const unsigned char **fread_pp, PgenReaderMain *pgrp, uintptr_t *__restrict genovec) {
    // Side effects:
...
PglErr LdLoadAndCopyGenovecSubset(const uintptr_t *__restrict sample_include, const uint32_t *__restrict sample_include_cumulative_popcounts,
                                  uint32_t sample_ct, uint32_t vidx, PgenReaderMain *pgrp, uintptr_t *dest) {
    const uint32_t raw_sample_ct = pgrp->fi.raw_sample_ct;
    if(LdLoadNecessary(vidx, pgrp)) {
        ...
        PglErr reterr =
            ParseNonLdGenovecSubsetUnsafe(fread_end, sample_include, sample_include_cumulative_popcounts, sample_ct, vrtype, &fread_ptr, pgrp, dest);
    ...
    if(pgrp->ldbase_stypes & kfPgrLdcacheNyp) {
        ...
        if((pgrp->ldbase_stypes & kfPgrLdcacheRawNyp) && (sample_ct == raw_sample_ct)) {
            ...
            PgrDifflistToGenovecUnsafe(pgrp->ldbase_raregeno, pgrp->ldbase_difflist_sample_ids, pgrp->fi.vrtypes[pgrp->ldbase_vidx] & 3, sample_ct,
                                       pgrp->ldbase_difflist_len, dest);
        } else {
...
//   two nonref = 2
//   missing = 3
PglErr ReadGenovecSubsetUnsafe(const uintptr_t *__restrict sample_include, const uint32_t *__restrict sample_include_cumulative_popcounts,
                               uint32_t sample_ct, uint32_t vidx, PgenReaderMain *pgrp, const unsigned char **fread_pp,
                               const unsigned char **fread_endp, uintptr_t *__restrict genovec) {
    // Side effects:
    ...
    PglErr reterr = ParseNonLdGenovecSubsetUnsafe(fread_end, sample_include, sample_include_cumulative_popcounts, sample_ct, maintrack_vrtype,
                                                  &fread_ptr, pgrp, genovec);
    if(unlikely(reterr)) {
        return reterr;
    }
...
PglErr PgrGet(const uintptr_t *__restrict sample_include, PgrSampleSubsetIndex pssi, uint32_t sample_ct, uint32_t vidx, PgenReader *pgr_ptr,
...
PglErr LdLoadAndCopyRawGenovec(uint32_t subsetting_required, uint32_t vidx, PgenReaderMain *pgrp, uintptr_t *dest) {
    ...
    if(pgrp->ldbase_stypes & kfPgrLdcacheRawNyp) {
        ...
        if(pgrp->ldbase_stypes & kfPgrLdcacheNyp) {
            ...
            PgrDifflistToGenovecUnsafe(pgrp->ldbase_raregeno, pgrp->ldbase_difflist_sample_ids, pgrp->fi.vrtypes[pgrp->ldbase_vidx] & 3,
                                       raw_sample_ct, pgrp->ldbase_difflist_len, dest);
        }
    }
    return kPglRetSuccess;
}
...
PglErr ReadRawGenovec(uint32_t subsetting_required, uint32_t vidx, PgenReaderMain *pgrp, const unsigned char **fread_pp,
                      const unsigned char **fread_endp, uintptr_t *raw_genovec) {
    const uint32_t vrtype = GetPgfiVrtype(&(pgrp->fi), vidx);
    const uint32_t maintrack_vrtype = vrtype & 7;
    const uint32_t raw_sample_ct = pgrp->fi.raw_sample_ct;
    if(VrtypeLdCompressed(maintrack_vrtype)) {
...
/*
void CopyAndSubsetDifflist(const uintptr_t* __restrict sample_include, const uint32_t* __restrict sample_include_cumulative_popcounts, const
uintptr_t* __restrict raw_raregeno, const uint32_t* __restrict raw_difflist_sample_ids, uint32_t raw_difflist_len, uintptr_t* __restrict new_raregeno,
uint32_t* __restrict new_difflist_sample_ids, uint32_t* __restrict new_difflist_len_ptr) {
...
PglErr LdLoadMinimalSubsetIfNecessary(const uintptr_t *__restrict sample_include, const uint32_t *__restrict sample_include_cumulative_popcounts,
                                      uint32_t sample_ct, uint32_t vidx, PgenReaderMain *pgrp) {
    if(!LdLoadNecessary(vidx, pgrp)) {
        return kPglRetSuccess;
    }
    const uint32_t ldbase_vidx = pgrp->ldbase_vidx;
    ...
    if(!subsetting_required) {
        reterr =
            ParseAndSaveDifflist(fread_end, raw_sample_ct, &fread_ptr, pgrp->ldbase_raregeno, pgrp->ldbase_difflist_sample_ids, &ldbase_difflist_len);
    } else {
        reterr = ParseAndSaveDifflistProperSubset(fread_end, sample_include, sample_include_cumulative_popcounts, raw_sample_ct, &fread_ptr,
                                                  pgrp->ldbase_raregeno, pgrp->ldbase_difflist_sample_ids, &ldbase_difflist_len,
                                                  pgrp->workspace_raregeno_tmp_loadbuf);
    }
    if(unlikely(reterr)) {
        return reterr;
    }
...
PglErr ReadDifflistOrGenovecSubsetUnsafe(const uintptr_t *__restrict sample_include, const uint32_t *__restrict sample_include_cumulative_popcounts,
                                         uint32_t sample_ct, uint32_t max_simple_difflist_len, uint32_t vidx, PgenReaderMain *pgrp,
                                         const unsigned char **fread_pp, const unsigned char **fread_endp, uintptr_t *__restrict genovec,
                                         uint32_t *difflist_common_geno_ptr, uintptr_t *__restrict main_raregeno,
                                         uint32_t *__restrict difflist_sample_ids, uint32_t *__restrict difflist_len_ptr) {
    assert(vidx < pgrp->fi.raw_variant_ct);
    assert(sample_ct);
    ...
    if(VrtypeLdCompressed(maintrack_vrtype)) {
        ...
        if(pgrp->ldbase_stypes & kfPgrLdcacheDifflist) {
            ...
            *difflist_common_geno_ptr = ldbase_common_geno;
            reterr = ParseLdAndMergeDifflistSubset(fread_end, subsetting_required ? sample_include : nullptr, sample_include_cumulative_popcounts,
                                                   pgrp->ldbase_raregeno, pgrp->ldbase_difflist_sample_ids, pgrp->ldbase_difflist_len,
                                                   ldbase_common_geno, raw_sample_ct, sample_ct, &fread_ptr, main_raregeno, difflist_sample_ids,
                                                   difflist_len_ptr, pgrp->workspace_raregeno_tmp_loadbuf);
            if(unlikely(reterr)) {
                return reterr;
            }
            if(ld_invert) {
    ...
    if(saved_difflist_len > max_simple_difflist_len) {
        *difflist_common_geno_ptr = UINT32_MAX;
        PglErr reterr = ParseNonLdGenovecSubsetUnsafe(fread_end, sample_include, sample_include_cumulative_popcounts, sample_ct, vrtype, &fread_ptr,
                                                      pgrp, genovec);
        if(unlikely(reterr)) {
            return reterr;
        }
    ...
    *difflist_common_geno_ptr = vrtype & 3;
    ...
    if(!subsetting_required) {
        ...
        reterr = ParseAndSaveDifflistProperSubset(fread_end, sample_include, sample_include_cumulative_popcounts, raw_sample_ct, &fread_ptr,
                                                  main_raregeno, difflist_sample_ids, difflist_len_ptr, pgrp->workspace_raregeno_tmp_loadbuf);
    }
    if(unlikely(reterr)) {
        return kPglRetMalformedInput;
    }
...
PglErr PgrGetDifflistOrGenovec(const uintptr_t *__restrict sample_include, PgrSampleSubsetIndex pssi, uint32_t sample_ct,
                               uint32_t max_simple_difflist_len, uint32_t vidx, PgenReader *pgr_ptr, uintptr_t *__restrict genovec,
                               uint32_t *difflist_common_geno_ptr, uintptr_t *__restrict main_raregeno, uint32_t *__restrict difflist_sample_ids,
                               uint32_t *__restrict difflist_len_ptr) {
    if(!sample_ct) {
        *difflist_common_geno_ptr = UINT32_MAX;
        return kPglRetSuccess;
    }
    PgenReaderMain *pgrp = GetPgrp(pgr_ptr);
    assert(vidx < pgrp->fi.raw_variant_ct);
    return ReadDifflistOrGenovecSubsetUnsafe(sample_include, GetSicp(pssi), sample_ct, max_simple_difflist_len, vidx, pgrp, nullptr, nullptr, genovec,
                                             difflist_common_geno_ptr, main_raregeno, difflist_sample_ids, difflist_len_ptr);
}

PglErr LdSubsetAdjustGenocounts(const unsigned char *fread_end, const uintptr_t *__restrict sample_include,
                                const uint32_t *__restrict sample_include_cumulative_popcounts, const uintptr_t *__restrict ldbase_genovec,
                                uint32_t raw_sample_ct, const unsigned char **fread_pp, STD_ARRAY_REF(uint32_t, 4) genocounts,
                                uintptr_t *__restrict raregeno_workspace) {
...
PglErr SkipDeltalistIds(const unsigned char *fread_end, const unsigned char *group_info, uint32_t difflist_len, uint32_t raw_sample_ct,
                        uint32_t has_genotypes, const unsigned char **fread_pp) {
    assert(difflist_len);
...
PglErr CountparseDifflistSubset(const unsigned char *fread_end, const uintptr_t *__restrict sample_include, uint32_t common_geno,
                                uint32_t raw_sample_ct, uint32_t sample_ct, const unsigned char **fread_pp, STD_ARRAY_REF(uint32_t, 4) genocounts,
                                uintptr_t *__restrict raregeno_workspace) {
    const unsigned char *group_info_iter;
    uint32_t difflist_len;
...
//   difflist
PglErr CountparseOnebitSubset(const unsigned char *fread_end, const uintptr_t *__restrict sample_include, uint32_t raw_sample_ct, uint32_t sample_ct,
                              const unsigned char **fread_pp, STD_ARRAY_REF(uint32_t, 4) genocounts, uintptr_t *__restrict raregeno_workspace) {
...
PglErr LdLoadGenovecSubsetIfNecessary(const uintptr_t *__restrict sample_include, const uint32_t *__restrict sample_include_cumulative_popcounts,
                                      uint32_t sample_ct, uint32_t vidx, PgenReaderMain *pgrp) {
    if(LdLoadNecessary(vidx, pgrp)) {
        const uint32_t ldbase_vidx = pgrp->ldbase_vidx;
        const unsigned char *fread_ptr;
        ...
        return ParseNonLdGenovecSubsetUnsafe(fread_end, sample_include, sample_include_cumulative_popcounts, sample_ct, vrtype, &fread_ptr, pgrp,
                                             pgrp->ldbase_genovec);
    }
    if(!(pgrp->ldbase_stypes & kfPgrLdcacheNyp)) {
        if(pgrp->ldbase_stypes & kfPgrLdcacheDifflist) {
            PgrDifflistToGenovecUnsafe(pgrp->ldbase_raregeno, pgrp->ldbase_difflist_sample_ids, pgrp->fi.vrtypes[pgrp->ldbase_vidx] & 3, sample_ct,
                                       pgrp->ldbase_difflist_len, pgrp->ldbase_genovec);
        } else {
            assert(pgrp->ldbase_stypes & kfPgrLdcacheRawNyp);
...
PglErr GetBasicGenotypeCounts(const uintptr_t *__restrict sample_include, const uintptr_t *__restrict sample_include_interleaved_vec,
                              const uint32_t *__restrict sample_include_cumulative_popcounts, uint32_t sample_ct, uint32_t vidx, PgenReaderMain *pgrp,
    ...
    if(VrtypeLdCompressed(vrtype)) {
        ...
        reterr = LdSubsetAdjustGenocounts(fread_end, subsetting_required ? sample_include : nullptr, sample_include_cumulative_popcounts,
                                          pgrp->ldbase_genovec, raw_sample_ct, &fread_ptr, genocounts, pgrp->workspace_raregeno_tmp_loadbuf);
        if(vrtype & 1) {
            // inverted
        ...
        if(is_ldbase) {
            ...
            reterr = ParseNonLdGenovecSubsetUnsafe(fread_end, sample_include, sample_include_cumulative_popcounts, sample_ct, vrtype, &fread_ptr,
                                                   pgrp, pgrp->ldbase_genovec);
            ZeroTrailingNyps(sample_ct, pgrp->ldbase_genovec);
            GenoarrCountFreqsUnsafe(pgrp->ldbase_genovec, sample_ct, genocounts);
            STD_ARRAY_COPY(genocounts, 4, pgrp->ldbase_basic_genocounts);
            pgrp->ldbase_stypes = (subsetting_required && (!(vrtype & 4))) ? (kfPgrLdcacheNyp | kfPgrLdcacheRawNyp | kfPgrLdcacheBasicGenocounts)
                                                                           : (kfPgrLdcacheNyp | kfPgrLdcacheBasicGenocounts);
        } else if(vrtype & 4) {
            const uint32_t vrtype_low2 = vrtype & 3;
            if(vrtype_low2 != 1) {
                reterr = CountparseDifflistSubset(fread_end, sample_include, vrtype & 3, raw_sample_ct, sample_ct, &fread_ptr, genocounts,
                                                  pgrp->workspace_raregeno_tmp_loadbuf);
            } else {
            ...
            reterr = CountparseOnebitSubset(fread_end, sample_include, raw_sample_ct, sample_ct, &fread_ptr, genocounts,
                                            pgrp->workspace_raregeno_tmp_loadbuf);
        } else {
...
PglErr PgrGetCounts(const uintptr_t *__restrict sample_include, const uintptr_t *__restrict sample_include_interleaved_vec, PgrSampleSubsetIndex pssi,
                    uint32_t sample_ct, uint32_t vidx, PgenReader *pgr_ptr, STD_ARRAY_REF(uint32_t, 4) genocounts) {
    if(!sample_ct) {
        STD_ARRAY_REF_FILL0(4, genocounts);
        return kPglRetSuccess;
    }
...
PglErr ParseAndSaveDeltalist(const unsigned char *fread_end, uint32_t raw_sample_ct, const unsigned char **fread_pp, uint32_t *__restrict deltalist,
                             uint32_t *__restrict deltalist_len_ptr) {
    const unsigned char *group_info_iter;
...
PglErr CountDeltalistIntersect(const unsigned char *fread_end, const uintptr_t *__restrict sample_include, uint32_t raw_sample_ct,
                               const unsigned char **fread_pp, uint32_t *__restrict intersect_ctp, uint32_t *__restrict raw_deltalist_len_ptr) {
...
// != 1.
// sample_include assumed to be nullptr if no subsetting required
PglErr CountAux1a(const unsigned char *fread_end, const uintptr_t *__restrict sample_include, const uintptr_t *__restrict raw_genoarr,
                  uint32_t aux1a_mode, uint32_t raw_sample_ct, uint32_t allele_ct, uint32_t allele_idx, uint32_t raw_01_ct, uint32_t subsetted_01_ct,
                  const unsigned char **fread_pp, uint32_t *__restrict het_ctp, uint32_t *__restrict deltalist_workspace) {
    if(aux1a_mode == 15) {
        if(allele_idx == 1) {
            if(sample_include) {
                ...
                for(uint32_t uii = 0; uii != loop_len; ++uii) {
                    ...
                    if(fset_bits & 1) {
                        if(rare01_lowbits == kBitsPerWord) {
                            ...
                            fvals_bits =
                                (detect_mask_hi & (~(fvals_bits | ((fvals_bits | detect_mask_hi) - detect_mask_lo)))) >> (allele_code_width - 1);
...
void CountAux1bDense(const void *patch_10_fvals, uint32_t allele_ct, uint32_t allele_idx_m1, uint32_t raw_10_ct, uint32_t rare10_ct,
                     uint32_t *__restrict het_ctp, uint32_t *__restrict hom_ctp) {
...
// != 1.
// sample_include assumed to be nullptr if no subsetting required
PglErr CountAux1b(const unsigned char *fread_end, const uintptr_t *__restrict sample_include, const uintptr_t *__restrict raw_genoarr,
                  uint32_t aux1b_mode, uint32_t raw_sample_ct, uint32_t allele_ct, uint32_t allele_idx, uint32_t raw_10_ct, uint32_t subsetted_10_ct,
                  const unsigned char **fread_pp, uint32_t *__restrict het_ctp, uint32_t *__restrict hom_ctp,
                  uint32_t *__restrict deltalist_workspace) {
    if(aux1b_mode == 15) {
        if(allele_idx == 1) {
            if(sample_include) {
...
#error "Unaligned accesses in CountAux1b()."
                ...
                for(uint32_t uii = 0; uii != loop_len; ++uii) {
                    ...
                    if(fset_bits & 1) {
                        if(rare10_lowbits == kBitsPerWord) {
                            ...
                            fvals_bits = (detect_alt1_mask_hi & (~(fvals_bits | ((fvals_bits | detect_alt1_mask_hi) - detect_hom_mask_lo)))) >>
                                         (allele_code_width - 1);
                            // unnecessary to apply bzhi here
                            ++fvals_widx;
                            rare10_lowbits = 0;
    ...
    if((!allele_idx_m1) || (allele_ct == 3)) {
        ...
        for(uint32_t fvals_widx = 0;; ++fvals_widx) {
            ...
            fvals_bits =
                (detect_alt1_mask_hi & (~(fvals_bits | ((fvals_bits | detect_alt1_mask_hi) - detect_hom_mask_lo)))) >> (allele_code_width - 1);
...
PglErr PgrGetInv1Counts(const uintptr_t *__restrict sample_include, const uintptr_t *__restrict sample_include_interleaved_vec,
                        PgrSampleSubsetIndex pssi, uint32_t sample_ct, uint32_t vidx, uint32_t allele_idx, PgenReader *pgr_ptr,
                        STD_ARRAY_REF(uint32_t, 4) genocounts) {
    ...
    PgrGetInv1Counts_biallelic:
        reterr = GetBasicGenotypeCounts(sample_include, sample_include_interleaved_vec, sample_include_cumulative_popcounts, sample_ct, vidx, pgrp,
                                        nullptr, genocounts);
        if(allele_idx) {
    ...
    uint32_t het_ct;
    reterr = CountAux1a(fread_end, sample_include, tmp_genovec, aux1a_mode, raw_sample_ct, allele_ct, allele_idx, raw_01_ct, subsetted_01_ct,
                        &fread_ptr, &het_ct, pgrp->workspace_difflist_sample_ids);
    if(unlikely(reterr)) {
        return reterr;
    }
    uint32_t hom_ct;
    reterr = CountAux1b(fread_end, sample_include, tmp_genovec, aux1b_mode, raw_sample_ct, allele_ct, allele_idx, raw_10_ct, subsetted_10_ct,
                        &fread_ptr, &het_ct, &hom_ct, pgrp->workspace_difflist_sample_ids);
    genocounts[0] = hom_ct;
    genocounts[1] = het_ct;
...
// sample_include assumed to be nullptr if no subsetting required
PglErr GenoarrAux1aUpdate(const unsigned char *fread_end, const uintptr_t *__restrict sample_include,
                          const uint32_t *__restrict sample_include_cumulative_popcounts, const uintptr_t *__restrict raw_genoarr,
                          uint32_t aux1a_mode, uint32_t raw_sample_ct, uint32_t allele_ct, uint32_t allele_idx, uintptr_t lshifted_bit,
                          uint32_t raw_01_ct, const unsigned char **fread_pp, uintptr_t *__restrict target_genoarr,
                          uint32_t *__restrict deltalist_workspace) {
    if(aux1a_mode == 15) {
        return kPglRetSuccess;
    }
                    ...
                    for(uint32_t uii = 0; uii != loop_len; ++uii) {
                        ...
                        if(fset_bits & 1) {
                            ...
                            if((R_CAST(const Halfword *, sample_include)[sample_hwidx]) & (1U << sample_uidx_lowbits)) {
                                const uint32_t sample_idx = RawToSubsettedPos(sample_include, sample_include_cumulative_popcounts,
                                                                              sample_hwidx * kBitsPerWordD2 + sample_uidx_lowbits);
...
// sample_include assumed to be nullptr if no subsetting required
PglErr GenoarrAux1bStandardUpdate(const unsigned char *fread_end, const uintptr_t *__restrict sample_include,
                                  const uint32_t *__restrict sample_include_cumulative_popcounts, const uintptr_t *__restrict raw_genoarr,
                                  uint32_t aux1b_mode, uint32_t raw_sample_ct, uint32_t allele_ct, uint32_t allele_idx, uint32_t raw_10_ct,
                    ...
                    for(uint32_t uii = 0; uii != loop_len; ++uii) {
                        ...
                        if(fset_bits & 1) {
                            if(rare10_lowbits == kBitsPerWord) {
                                ...
                                if(!allele_idx_m1) {
                                    fvals_bits =
                                        (detect_alt1_mask_hi & (~(fvals_bits | ((fvals_bits | detect_alt1_mask_hi) - detect_hom_mask_lo)))) >>
                                        (allele_code_width - 1);
                                }
                                // unnecessary to apply bzhi here
                                ++fvals_widx;
                ...
                for(uint32_t uii = 0; uii != loop_len; ++uii) {
                    ...
                    if(sample_include_word & lowbit) {
                        const uint32_t sample_idx =
                            sample_include_cumulative_popcounts[sample_widx] + PopcountWord(sample_include_word & (lowbit - 1));
                        const uintptr_t cur_lowcode0 = fvals_bits & 1;
                        const uintptr_t shifted_xor_mult = (lowcode_add + cur_lowcode0) << (2 * (sample_idx % kBitsPerWordD2));
                        target_genoarr[sample_idx / kBitsPerWordD2] ^= shifted_xor_mult;
                    }
...
PglErr GetAux1bHets(const unsigned char *fread_end, const uintptr_t *__restrict raw_genoarr, uint32_t aux1b_mode, uint32_t raw_sample_ct,
                    uint32_t allele_ct, uint32_t raw_10_ct, const unsigned char **fread_pp, uintptr_t *__restrict aux1b_hets,
                ...
                if(fset_bits & 1) {
                    if(rare10_lowbits == kBitsPerWord) {
                        ...
                        if(allele_ct == 3) {
                            ...
                            fvals_bits =
                                detect_hom_mask_lo & ((fvals_bits | ((fvals_bits | detect_all_mask_hi) - detect_all_mask_lo)) >> (code10_width - 1));
                        }
...
PglErr Get1Multiallelic(const uintptr_t *__restrict sample_include, const uint32_t *__restrict sample_include_cumulative_popcounts,
                        uint32_t sample_ct, uint32_t vidx, uint32_t allele_idx, PgenReaderMain *pgrp, const unsigned char **fread_pp,
                        const unsigned char **fread_endp, uintptr_t *__restrict all_hets, uintptr_t *__restrict allele_countvec,
                        uintptr_t **subsetted_10hetp) {
    ...
    reterr = GenoarrAux1aUpdate(fread_end, sample_include, sample_include_cumulative_popcounts, raw_genovec, aux1a_mode, raw_sample_ct, allele_ct,
                                allele_idx, 1, raw_01_ct, &fread_ptr, allele_countvec, deltalist_workspace);
    if(unlikely(reterr)) {
        return reterr;
    }
    const unsigned char *aux1b_start = fread_ptr;
    reterr = GenoarrAux1bStandardUpdate(fread_end, sample_include, sample_include_cumulative_popcounts, raw_genovec, aux1b_mode, raw_sample_ct,
                                        allele_ct, allele_idx, raw_10_ct, &fread_ptr, allele_countvec, deltalist_workspace);
    if((!fread_pp) || reterr) {
        return reterr;
    }
    *fread_pp = fread_ptr;
    if(all_hets) {
        ...
        reterr = GetAux1bHets(fread_end, raw_genovec, aux1b_mode, raw_sample_ct, allele_ct, raw_10_ct, &aux1b_start, aux1b_hets, &aux1b_het_present,
                              deltalist_workspace);
        if(unlikely(reterr)) {
            return reterr;
        }
...
PglErr IMPLPgrGet1(const uintptr_t *__restrict sample_include, const uint32_t *__restrict sample_include_cumulative_popcounts, uint32_t sample_ct,
                   uint32_t vidx, uint32_t allele_idx, PgenReaderMain *pgrp, uintptr_t *__restrict allele_countvec) {
    if(!sample_ct) {
        return kPglRetSuccess;
    }
    const uint32_t vrtype = GetPgfiVrtype(&(pgrp->fi), vidx);
    ...
    if((!allele_idx) || ((allele_idx == 1) && (!multiallelic_hc_present))) {
        PglErr reterr =
            ReadGenovecSubsetUnsafe(sample_include, sample_include_cumulative_popcounts, sample_ct, vidx, pgrp, nullptr, nullptr, allele_countvec);
        if(unlikely(reterr)) {
            return reterr;
        }
        if(!allele_idx) {
    ...
    return Get1Multiallelic(sample_include, sample_include_cumulative_popcounts, sample_ct, vidx, allele_idx, pgrp, nullptr, nullptr, nullptr,
                            allele_countvec, nullptr);
}

PglErr IMPLPgrGetInv1(const uintptr_t *__restrict sample_include, const uint32_t *__restrict sample_include_cumulative_popcounts, uint32_t sample_ct,
                      uint32_t vidx, uint32_t allele_idx, PgenReaderMain *pgrp, uintptr_t *__restrict allele_invcountvec) {
    if(!sample_ct) {
        return kPglRetSuccess;
    }
    const uint32_t vrtype = GetPgfiVrtype(&(pgrp->fi), vidx);
    ...
    if((!allele_idx) || ((allele_idx == 1) && (!multiallelic_hc_present))) {
        PglErr reterr =
            ReadGenovecSubsetUnsafe(sample_include, sample_include_cumulative_popcounts, sample_ct, vidx, pgrp, nullptr, nullptr, allele_invcountvec);
        if(unlikely(reterr)) {
            return reterr;
        }
        if(allele_idx) {
    ...
    PglErr reterr = Get1Multiallelic(sample_include, sample_include_cumulative_popcounts, sample_ct, vidx, allele_idx, pgrp, nullptr, nullptr,
                                     nullptr, allele_invcountvec, nullptr);
    GenovecInvertUnsafe(sample_ct, allele_invcountvec);
    return reterr;
}
...
PglErr SkipAux1a(const unsigned char *fread_end, uint32_t aux1a_mode, uint32_t raw_sample_ct, uint32_t allele_ct, uint32_t raw_01_ct,
...
// sample_include assumed to be nullptr if no subsetting required
PglErr GenoarrAux1bUpdate2(const unsigned char *fread_end, const uintptr_t *__restrict sample_include,
                           const uint32_t *__restrict sample_include_cumulative_popcounts, const uintptr_t *__restrict raw_genoarr,
                           uint32_t aux1b_mode, uint32_t raw_sample_ct, uint32_t allele_ct, uint32_t allele_idx0, uint32_t allele_idx1,
                           uint32_t raw_10_ct, const unsigned char **fread_pp, uintptr_t *__restrict target_genoarr,
                           uint32_t *__restrict deltalist_workspace) {
    ...
    if(allele_idx0 == 1) {
        return GenoarrAux1bStandardUpdate(fread_end, sample_include, sample_include_cumulative_popcounts, raw_genoarr, aux1b_mode, raw_sample_ct,
                                          allele_ct, allele_idx1, raw_10_ct, fread_pp, target_genoarr, deltalist_workspace);
    }
    uintptr_t detect_hom_mask_lo;
    const uint32_t allele_code_logwidth = GetAux1bConsts(allele_ct, &detect_hom_mask_lo);
    const uint32_t code10_logwidth = allele_code_logwidth + (allele_code_logwidth != 0);
...
PglErr IMPLPgrGet2(const uintptr_t *__restrict sample_include, const uint32_t *__restrict sample_include_cumulative_popcounts, uint32_t sample_ct,
                   uint32_t vidx, uint32_t allele_idx0, uint32_t allele_idx1, PgenReaderMain *pgrp, uintptr_t *__restrict genovec) {
    ...
    if(!multiallelic_hc_present) {
        ...
        PglErr reterr =
            ReadGenovecSubsetUnsafe(sample_include, sample_include_cumulative_popcounts, sample_ct, vidx, pgrp, nullptr, nullptr, genovec);
        if(unlikely(reterr)) {
            return reterr;
        }
    ...
    if(!allele_idx0) {
        ...
        reterr = GenoarrAux1aUpdate(fread_end, sample_include, sample_include_cumulative_popcounts, raw_genovec, aux1a_mode, raw_sample_ct, allele_ct,
                                    allele_idx1, 2, raw_01_ct, &fread_ptr, genovec, deltalist_workspace);
    } else {
    ...
    reterr = GenoarrAux1bUpdate2(fread_end, sample_include, sample_include_cumulative_popcounts, raw_genovec, aux1b_mode, raw_sample_ct, allele_ct,
                                 allele_idx0, allele_idx1, raw_10_ct, &fread_ptr, genovec, deltalist_workspace);
    if(unlikely(reterr)) {
        return reterr;
    }
    if(invert) {
...
// similar to ParseAndSaveDifflist()
PglErr ParseAndSaveDeltalistAsBitarr(const unsigned char *fread_end, uint32_t raw_sample_ct, const unsigned char **fread_pp,
                                     uintptr_t *deltalist_include, uint32_t *deltalist_len_ptr) {
    const unsigned char *group_info_iter;
    PglErr reterr = ParseDifflistHeader(fread_end, raw_sample_ct, fread_pp, nullptr, &group_info_iter, deltalist_len_ptr);
    const uint32_t deltalist_len = *deltalist_len_ptr;
    if(reterr || (!deltalist_len)) {
...
PglErr GetAux1aCodes(const unsigned char *fread_end, uint32_t rare01_ct, uint32_t allele_ct, const unsigned char **fread_pp,
                     AlleleCode *__restrict patch_01_vals) {
    if(allele_ct == 3) {
...
PglErr ExportAux1a(const unsigned char *fread_end, const uintptr_t *__restrict raw_genoarr, uint32_t aux1a_mode, uint32_t raw_sample_ct,
                   uint32_t allele_ct, uint32_t raw_01_ct, const unsigned char **fread_pp, uintptr_t *__restrict patch_01_set,
                   AlleleCode *__restrict patch_01_vals, uint32_t *__restrict rare01_ctp) {
    uint32_t rare01_ct;
    if(!aux1a_mode) {
...
PglErr ExportAux1aProperSubset(const unsigned char *fread_end, const uintptr_t *__restrict sample_include,
                               const uint32_t *__restrict sample_include_cumulative_popcounts, const uintptr_t *__restrict raw_genoarr,
                               uint32_t aux1a_mode, uint32_t raw_sample_ct, uint32_t sample_ct, uint32_t allele_ct, uint32_t raw_01_ct,
                               const unsigned char **fread_pp, uintptr_t *__restrict dst_01_set, AlleleCode *__restrict dst_01_vals,
...
PglErr GetAux1bCodes(const unsigned char *fread_end, uint32_t rare10_ct, uint32_t allele_ct, const unsigned char **fread_pp,
                     AlleleCode *__restrict patch_10_vals) {
    const unsigned char *patch_10_fvals = *fread_pp;
    if(allele_ct == 3) {
...
PglErr ExportAux1b(const unsigned char *fread_end, const uintptr_t *__restrict raw_genoarr, uint32_t aux1b_mode, uint32_t raw_sample_ct,
                   uint32_t allele_ct, uint32_t raw_10_ct, const unsigned char **fread_pp, uintptr_t *__restrict patch_10_set,
                   AlleleCode *__restrict patch_10_vals, uint32_t *__restrict rare10_ctp) {
    uint32_t rare10_ct;
    if(!aux1b_mode) {
...
PglErr ExportAux1bProperSubset(const unsigned char *fread_end, const uintptr_t *__restrict sample_include,
                               const uint32_t *__restrict sample_include_cumulative_popcounts, const uintptr_t *__restrict raw_genoarr,
                               uint32_t aux1b_mode, uint32_t raw_sample_ct, uint32_t sample_ct, uint32_t allele_ct, uint32_t raw_10_ct,
                               const unsigned char **fread_pp, uintptr_t *__restrict dst_10_set, AlleleCode *__restrict dst_10_vals,
                               uint32_t *__restrict rare10_ctp, uint32_t *__restrict deltalist_workspace) {
...
PglErr GetMultiallelicCodes(const uintptr_t *__restrict sample_include, const uint32_t *__restrict sample_include_cumulative_popcounts,
                            uint32_t sample_ct, uint32_t vidx, PgenReaderMain *pgrp, const unsigned char **fread_pp, const unsigned char **fread_endp,
                            uintptr_t *__restrict all_hets, PgenVariant *pgvp) {
    const uint32_t raw_sample_ct = pgrp->fi.raw_sample_ct;
    ...
    if(aux1a_mode != 15) {
        if(!subsetting_required) {
            reterr = ExportAux1a(fread_end, raw_genovec, aux1a_mode, raw_sample_ct, allele_ct, raw_01_ct, &fread_ptr, pgvp->patch_01_set,
                                 pgvp->patch_01_vals, &(pgvp->patch_01_ct));
        } else {
            reterr = ExportAux1aProperSubset(fread_end, sample_include, sample_include_cumulative_popcounts, raw_genovec, aux1a_mode, raw_sample_ct,
                                             sample_ct, allele_ct, raw_01_ct, &fread_ptr, pgvp->patch_01_set, pgvp->patch_01_vals,
                                             &(pgvp->patch_01_ct), deltalist_workspace);
        }
        if(unlikely(reterr)) {
            return reterr;
        }
    ...
    if(aux1b_mode != 15) {
        if(!subsetting_required) {
            reterr = ExportAux1b(fread_end, raw_genovec, aux1b_mode, raw_sample_ct, allele_ct, raw_10_ct, &fread_ptr, pgvp->patch_10_set,
                                 pgvp->patch_10_vals, &(pgvp->patch_10_ct));
        } else {
            reterr = ExportAux1bProperSubset(fread_end, sample_include, sample_include_cumulative_popcounts, raw_genovec, aux1b_mode, raw_sample_ct,
                                             sample_ct, allele_ct, raw_10_ct, &fread_ptr, pgvp->patch_10_set, pgvp->patch_10_vals,
                                             &(pgvp->patch_10_ct), deltalist_workspace);
        }
        if(unlikely(reterr)) {
            return reterr;
        }
        ...
        *fread_endp = fread_end;
        if(all_hets) {
            ...
            if(aux1b_mode != 15) {
                ...
                uintptr_t *aux1b_hets = pgrp->workspace_aux1x_present;
                uint32_t aux1b_het_present;
                reterr = GetAux1bHets(fread_end, raw_genovec, aux1b_mode, raw_sample_ct, allele_ct, raw_10_ct, &aux1b_start, aux1b_hets,
                                      &aux1b_het_present, deltalist_workspace);
                if(unlikely(reterr)) {
                    return reterr;
                }
                if(aux1b_het_present) {
...
PglErr PgrGetM(const uintptr_t *__restrict sample_include, PgrSampleSubsetIndex pssi, uint32_t sample_ct, uint32_t vidx, PgenReader *pgr_ptr,
               PgenVariant *pgvp) {
    pgvp->patch_01_ct = 0;
    pgvp->patch_10_ct = 0;
    if(!sample_ct) {
...
void PgrDetectGenoarrHetsMultiallelic(const uintptr_t *__restrict genoarr, const uintptr_t *__restrict patch_10_set,
                                      const AlleleCode *__restrict patch_10_vals, uint32_t raw_sample_ct, uintptr_t *__restrict all_hets) {
...
PglErr SkipAux1b(const unsigned char *fread_end, uint32_t aux1b_mode, uint32_t raw_sample_ct, uint32_t allele_ct, uint32_t raw_10_ct,
                 const unsigned char **fread_pp) {
    if(aux1b_mode == 15) {
        return kPglRetSuccess;
    }
...
PglErr SkipAux1(const unsigned char *fread_end, const uintptr_t *__restrict raw_genovec, uint32_t raw_sample_ct, uint32_t allele_ct,
                const unsigned char **fread_pp) {
...
PglErr ParseAux2Subset(const unsigned char *fread_end, const uintptr_t *__restrict sample_include, const uintptr_t *__restrict all_hets,
                       const uintptr_t *__restrict subsetted_10het, uint32_t raw_sample_ct, uint32_t sample_ct, const unsigned char **fread_pp,
                       uintptr_t *__restrict phasepresent, uintptr_t *__restrict phaseinfo, uint32_t *__restrict phasepresent_ct_ptr,
                       uintptr_t *__restrict workspace_subset) {
    const uint32_t raw_sample_ctl = BitCtToWordCt(raw_sample_ct);
    ...
    if(!(aux2_start[0] & 1)) {
        ...
        *fread_pp = aux2_second_part;
        ...
        if(!sample_include) {
            ...
            ExpandThenSubsetBytearrNested(aux2_second_part, aux2_first_part_copy, all_hets, sample_include, sample_ct, raw_phasepresent_ct, 1,
...
PglErr ReadGenovecHphaseSubsetUnsafe(const uintptr_t *__restrict sample_include, const uint32_t *__restrict sample_include_cumulative_popcounts,
                                     uint32_t sample_ct, uint32_t vidx, PgenReaderMain *pgrp, const unsigned char **fread_pp,
                                     const unsigned char **fread_endp, uintptr_t *__restrict genovec, uintptr_t *__restrict phasepresent,
                                     uintptr_t *__restrict phaseinfo, uint32_t *phasepresent_ct_ptr) {
    const uint32_t vrtype = GetPgfiVrtype(&(pgrp->fi), vidx);
    ...
    if(VrtypeMultiallelicHc(vrtype)) {
        ...
        uint32_t *deltalist_workspace = pgrp->workspace_difflist_sample_ids;
        uint32_t aux1b_het_present;
        reterr = GetAux1bHets(fread_end, raw_genovec, aux1b_mode, raw_sample_ct, allele_ct, raw_10_ct, &fread_ptr, aux1b_hets, &aux1b_het_present,
                              deltalist_workspace);
        if(unlikely(reterr)) {
            return reterr;
        }
    ...
    reterr = ParseAux2Subset(fread_end, subsetting_required ? sample_include : nullptr, all_hets, subsetted_10het, raw_sample_ct, sample_ct,
                             &fread_ptr, phasepresent, phaseinfo, phasepresent_ct_ptr, pgrp->workspace_subset);
    if(fread_pp) {
        *fread_pp = fread_ptr;
        *fread_endp = fread_end;
    }
...
PglErr PgrGetP(const uintptr_t *__restrict sample_include, PgrSampleSubsetIndex pssi, uint32_t sample_ct, uint32_t vidx, PgenReader *pgr_ptr,
               uintptr_t *__restrict genovec, uintptr_t *__restrict phasepresent, uintptr_t *__restrict phaseinfo,
               uint32_t *__restrict phasepresent_ct_ptr) {
    if(!sample_ct) {
        *phasepresent_ct_ptr = 0;
        return kPglRetSuccess;
    }
    PgenReaderMain *pgrp = GetPgrp(pgr_ptr);
    assert(vidx < pgrp->fi.raw_variant_ct);
    return ReadGenovecHphaseSubsetUnsafe(sample_include, GetSicp(pssi), sample_ct, vidx, pgrp, nullptr, nullptr, genovec, phasepresent, phaseinfo,
                                         phasepresent_ct_ptr);
}

// eventually want to return fread_ptr/fread_end, but not relevant until
// multiallelic dosage working
PglErr Get1MP(const uintptr_t *__restrict sample_include, const uint32_t *__restrict sample_include_cumulative_popcounts, uint32_t sample_ct,
              uint32_t vidx, uint32_t allele_idx, PgenReaderMain *pgrp, uintptr_t *__restrict allele_countvec, uintptr_t *__restrict phasepresent,
              uintptr_t *__restrict phaseinfo, uint32_t *__restrict phasepresent_ct_ptr) {
    // sample_ct > 0; either allele_idx > 1 or ((allele_idx == 1) &&
    // multiallelic_hc_present)
    const uint32_t vrtype = pgrp->fi.vrtypes[vidx];
    if(!VrtypeHphase(vrtype)) {
    ...
    const unsigned char *fread_ptr;
    const unsigned char *fread_end;
    PglErr reterr = Get1Multiallelic(sample_include, sample_include_cumulative_popcounts, sample_ct, vidx, allele_idx, pgrp, &fread_ptr, &fread_end,
                                     all_hets, allele_countvec, &subsetted_10het);
    if(unlikely(reterr)) {
        return reterr;
    }
    const uint32_t raw_sample_ct = pgrp->fi.raw_sample_ct;
    reterr = ParseAux2Subset(fread_end, (sample_ct != raw_sample_ct) ? sample_include : nullptr, all_hets, subsetted_10het, raw_sample_ct, sample_ct,
                             &fread_ptr, phasepresent, phaseinfo, phasepresent_ct_ptr, pgrp->workspace_subset);
...
PglErr PgrGet1P(const uintptr_t *__restrict sample_include, PgrSampleSubsetIndex pssi, uint32_t sample_ct, uint32_t vidx, uint32_t allele_idx,
                PgenReader *pgr_ptr, uintptr_t *__restrict allele_countvec, uintptr_t *__restrict phasepresent, uintptr_t *__restrict phaseinfo,
                uint32_t *__restrict phasepresent_ct_ptr) {
    if(!sample_ct) {
        *phasepresent_ct_ptr = 0;
        return kPglRetSuccess;
    ...
    const uint32_t multiallelic_hc_present = VrtypeMultiallelicHc(vrtype);
    if((!allele_idx) || ((allele_idx == 1) && (!multiallelic_hc_present))) {
        PglErr reterr = ReadGenovecHphaseSubsetUnsafe(sample_include, sample_include_cumulative_popcounts, sample_ct, vidx, pgrp, nullptr, nullptr,
                                                      allele_countvec, phasepresent, phaseinfo, phasepresent_ct_ptr);
        if(allele_idx) {
            GenovecInvertUnsafe(sample_ct, allele_countvec);
    ...
    return Get1MP(sample_include, sample_include_cumulative_popcounts, sample_ct, vidx, allele_idx, pgrp, allele_countvec, phasepresent, phaseinfo,
                  phasepresent_ct_ptr);
}

PglErr IMPLPgrGetInv1P(const uintptr_t *__restrict sample_include, const uint32_t *__restrict sample_include_cumulative_popcounts, uint32_t sample_ct,
                       uint32_t vidx, uint32_t allele_idx, PgenReaderMain *pgrp, uintptr_t *__restrict allele_invcountvec,
                       uintptr_t *__restrict phasepresent, uintptr_t *__restrict phaseinfo, uint32_t *__restrict phasepresent_ct_ptr) {
    if(!sample_ct) {
        *phasepresent_ct_ptr = 0;
    ...
    if((!allele_idx) || ((allele_idx == 1) && (!multiallelic_hc_present))) {
        PglErr reterr = ReadGenovecHphaseSubsetUnsafe(sample_include, sample_include_cumulative_popcounts, sample_ct, vidx, pgrp, nullptr, nullptr,
                                                      allele_invcountvec, phasepresent, phaseinfo, phasepresent_ct_ptr);
        if(!allele_idx) {
            GenovecInvertUnsafe(sample_ct, allele_invcountvec);
            if(*phasepresent_ct_ptr) {
                BitvecInvert(BitCtToWordCt(sample_ct), phaseinfo);
    ...
    PglErr reterr = Get1MP(sample_include, sample_include_cumulative_popcounts, sample_ct, vidx, allele_idx, pgrp, allele_invcountvec, phasepresent,
                           phaseinfo, phasepresent_ct_ptr);
    if(unlikely(reterr)) {
        return reterr;
    }
    GenovecInvertUnsafe(sample_ct, allele_invcountvec);
...
PglErr PgrGet2P(const uintptr_t *__restrict sample_include, PgrSampleSubsetIndex pssi, uint32_t sample_ct, uint32_t vidx, uint32_t allele_idx0,
                uint32_t allele_idx1, PgenReader *pgr_ptr, uintptr_t *__restrict genovec, uintptr_t *__restrict phasepresent,
                uintptr_t *__restrict phaseinfo, uint32_t *__restrict phasepresent_ct_ptr) {
    PgenReaderMain *pgrp = GetPgrp(pgr_ptr);
    const uint32_t *sample_include_cumulative_popcounts = GetSicp(pssi);
    const uint32_t vrtype = GetPgfiVrtype(&(pgrp->fi), vidx);
    ...
    if(allele_idx0 + allele_idx1 == 1) {
        PglErr reterr = ReadGenovecHphaseSubsetUnsafe(sample_include, sample_include_cumulative_popcounts, sample_ct, vidx, pgrp, nullptr, nullptr,
                                                      genovec, phasepresent, phaseinfo, phasepresent_ct_ptr);
        if(allele_idx0) {
            GenovecInvertUnsafe(sample_ct, genovec);
            if(*phasepresent_ct_ptr) {
                BitvecInvert(BitCtToWordCt(sample_ct), phaseinfo);
    ...
    if(VrtypeMultiallelicHc(vrtype)) {
        ...
        if(!allele_idx0) {
            ...
            // - If allele_idx == 1, convert all aux1a entries from 01 to 11.
            // - Otherwise, for each matching aux1a entry, convert from 11 to 01.
            reterr = GenoarrAux1aUpdate(fread_end, sample_include, sample_include_cumulative_popcounts, raw_genovec, aux1a_mode, raw_sample_ct,
                                        allele_ct, allele_idx1, 2, raw_01_ct, &fread_ptr, genovec, deltalist_workspace);
        } else {
            reterr = SkipAux1a(fread_end, aux1a_mode, raw_sample_ct, allele_ct, raw_01_ct, &fread_ptr);
        }
        if(unlikely(reterr)) {
        ...
        const unsigned char *aux1b_start = fread_ptr;
        reterr = GenoarrAux1bUpdate2(fread_end, sample_include, sample_include_cumulative_popcounts, raw_genovec, aux1b_mode, raw_sample_ct,
                                     allele_ct, allele_idx0, allele_idx1, raw_10_ct, &fread_ptr, genovec, deltalist_workspace);
        if(unlikely(reterr)) {
            return reterr;
        }
    ...
    reterr = ParseAux2Subset(fread_end, sample_include, all_hets, subsetted_10het, raw_sample_ct, sample_ct, &fread_ptr, phasepresent, phaseinfo,
                             phasepresent_ct_ptr, pgrp->workspace_subset);
    if(unlikely(reterr)) {
        return reterr;
    }
...
PglErr PgrGetMP(const uintptr_t *__restrict sample_include, PgrSampleSubsetIndex pssi, uint32_t sample_ct, uint32_t vidx, PgenReader *pgr_ptr,
                PgenVariant *pgvp) {
    pgvp->patch_01_ct = 0;
    pgvp->patch_10_ct = 0;
    if(!sample_ct) {
    ...
    const uint32_t multiallelic_hc_present = VrtypeMultiallelicHc(vrtype);
    if(!multiallelic_hc_present) {
        return ReadGenovecHphaseSubsetUnsafe(sample_include, sample_include_cumulative_popcounts, sample_ct, vidx, pgrp, nullptr, nullptr,
                                             pgvp->genovec, pgvp->phasepresent, pgvp->phaseinfo, &(pgvp->phasepresent_ct));
    }
    const unsigned char *fread_ptr;
    const unsigned char *fread_end;
    uintptr_t *all_hets = VrtypeHphase(vrtype) ? pgrp->workspace_all_hets : nullptr;
    PglErr reterr = GetMultiallelicCodes(sample_include, sample_include_cumulative_popcounts, sample_ct, vidx, pgrp,
                                         all_hets ? (&fread_ptr) : nullptr, all_hets ? (&fread_end) : nullptr, all_hets, pgvp);
    if(reterr || (!all_hets)) {
        return reterr;
    }
    const uint32_t raw_sample_ct = pgrp->fi.raw_sample_ct;
    return ParseAux2Subset(fread_end, (sample_ct != raw_sample_ct) ? sample_include : nullptr, all_hets, nullptr, raw_sample_ct, sample_ct,
                           &fread_ptr, pgvp->phasepresent, pgvp->phaseinfo, &(pgvp->phasepresent_ct), pgrp->workspace_subset);
}

// ok for sample_include to be nullptr if not subsetting, though this is not
// required
PglErr ParseDosage16(const unsigned char *fread_ptr, const unsigned char *fread_end, const uintptr_t *__restrict sample_include, uint32_t sample_ct,
                     uint32_t vidx, uint32_t allele_ct, PgenReaderMain *pgrp, uint32_t *__restrict dosage_ct_ptr,
                     uintptr_t *__restrict dphase_present, int16_t *dphase_delta, uint32_t *__restrict dphase_ct_ptr,
                     uintptr_t *__restrict dosage_present, uint16_t *dosage_main) {
...
PglErr IMPLPgrGetD(const uintptr_t *__restrict sample_include, const uint32_t *__restrict sample_include_cumulative_popcounts, uint32_t sample_ct,
                   uint32_t vidx, PgenReaderMain *pgrp, uintptr_t *__restrict genovec, uintptr_t *__restrict dosage_present, uint16_t *dosage_main,
                   uint32_t *dosage_ct_ptr) {
    assert(vidx < pgrp->fi.raw_variant_ct);
    if(!sample_ct) {
        *dosage_ct_ptr = 0;
        return kPglRetSuccess;
    ...
    PglErr reterr = ReadGenovecHphaseSubsetUnsafe(sample_include, sample_include_cumulative_popcounts, sample_ct, vidx, pgrp, &fread_ptr, &fread_end,
                                                  genovec, nullptr, nullptr, &phasepresent_ct);
    if(unlikely(reterr)) {
        return reterr;
    }
    const uintptr_t *allele_idx_offsets = pgrp->fi.allele_idx_offsets;
    const uint32_t allele_ct = allele_idx_offsets ? (allele_idx_offsets[vidx + 1] - allele_idx_offsets[vidx]) : 2;
    return ParseDosage16(fread_ptr, fread_end, sample_include, sample_ct, vidx, allele_ct, pgrp, dosage_ct_ptr, nullptr, nullptr, nullptr,
                         dosage_present, dosage_main);
}

PglErr PgrGet1D(const uintptr_t *__restrict sample_include, PgrSampleSubsetIndex pssi, uint32_t sample_ct, uint32_t vidx, AlleleCode allele_idx,
                PgenReader *pgr_ptr, uintptr_t *__restrict allele_countvec, uintptr_t *__restrict dosage_present, uint16_t *dosage_main,
                uint32_t *dosage_ct_ptr) {
    PgenReaderMain *pgrp = GetPgrp(pgr_ptr);
    const uint32_t *sample_include_cumulative_popcounts = GetSicp(pssi);
    const uintptr_t *allele_idx_offsets = pgrp->fi.allele_idx_offsets;
    ...
    if((allele_ct == 2) || (!allele_idx)) {
        uint32_t dosage_ct;
        PglErr reterr = IMPLPgrGetD(sample_include, sample_include_cumulative_popcounts, sample_ct, vidx, pgrp, allele_countvec, dosage_present,
                                    dosage_main, &dosage_ct);
        //printf("PgrGetD %lu\n", reterr);
        if(!allele_idx) {
            GenovecInvertUnsafe(sample_ct, allele_countvec);
            if(dosage_ct) {
...
PglErr PgrGetInv1D(const uintptr_t *__restrict sample_include, PgrSampleSubsetIndex pssi, uint32_t sample_ct, uint32_t vidx, AlleleCode allele_idx,
                   PgenReader *pgr_ptr, uintptr_t *__restrict allele_invcountvec, uintptr_t *__restrict dosage_present, uint16_t *dosage_main,
                   uint32_t *dosage_ct_ptr) {
    PgenReaderMain *pgrp = GetPgrp(pgr_ptr);
    const uint32_t *sample_include_cumulative_popcounts = GetSicp(pssi);
    const uintptr_t *allele_idx_offsets = pgrp->fi.allele_idx_offsets;
    ...
    if((allele_ct == 2) || (!allele_idx)) {
        uint32_t dosage_ct;
        PglErr reterr = IMPLPgrGetD(sample_include, sample_include_cumulative_popcounts, sample_ct, vidx, pgrp, allele_invcountvec, dosage_present,
                                    dosage_main, &dosage_ct);
        if(allele_idx) {
            GenovecInvertUnsafe(sample_ct, allele_invcountvec);
            if(dosage_ct) {
...
PglErr GetAux1bHetIncr(const unsigned char *fread_end, uint32_t aux1b_mode, uint32_t raw_sample_ct, uint32_t allele_ct, uint32_t raw_10_ct,
                       const unsigned char **fread_pp, uint32_t *__restrict raw_het_ctp) {
    if(aux1b_mode == 15) {
        return kPglRetSuccess;
    }
    uint32_t rare10_ct;
...
PglErr GetPhasepresentAndSkipPhaseinfo(const unsigned char *fread_end, const uintptr_t *__restrict all_hets, uint32_t raw_sample_ct, uint32_t het_ct,
                                       const unsigned char **fread_pp, uintptr_t *__restrict phasepresent, uint32_t *__restrict phasepresent_ctp) {
    const unsigned char *aux2_start = *fread_pp;
    const uint32_t aux2_first_part_byte_ct = 1 + (het_ct / CHAR_BIT);
    if(PtrAddCk(fread_end, aux2_first_part_byte_ct, fread_pp)) {
        return kPglRetMalformedInput;
...
PglErr GetUnphasedBiallelicHetCt(const uintptr_t *__restrict sample_include, const uintptr_t *raw_genoarr, const unsigned char *fread_ptr,
                                 const unsigned char *fread_end, uint32_t subsetted_het_ct, PgenReaderMain *pgrp, uint32_t *unphased_het_ctp) {
    const uint32_t raw_sample_ct = pgrp->fi.raw_sample_ct;
    uint32_t raw_het_ct;
    if(!sample_include) {
...
PglErr GetPhasedBiallelicGenotypeSubsetCounts(const uintptr_t *__restrict sample_include, const uintptr_t *__restrict sample_include_interleaved_vec,
                                              uint32_t sample_ct, uint32_t vidx, PgenReaderMain *pgrp, uint32_t *unphased_het_ctp,
                                              STD_ARRAY_REF(uint32_t, 4) genocounts) {
...
//   notes.
PglErr GetBasicGenotypeCountsAndDosage16s(const uintptr_t *__restrict sample_include, const uintptr_t *__restrict sample_include_interleaved_vec,
                                          const uint32_t *__restrict sample_include_cumulative_popcounts, uint32_t sample_ct, uint32_t vidx,
                                          uint32_t is_minimac3_r2, PgenReaderMain *pgrp, double *imp_r2_ptr, STD_ARRAY_REF(uint32_t, 4) genocounts,
                                          uint64_t *__restrict all_dosages) {
    // genocounts[0] := ref/ref, genocounts[1] := ref/altx,
    // genocounts[2] := altx/alty, genocounts[3] := missing
    const uint32_t vrtype = GetPgfiVrtype(&(pgrp->fi), vidx);
    const uint32_t raw_sample_ct = pgrp->fi.raw_sample_ct;
    ...
    // otherwise never.)
    if((!(pgrp->fi.gflags & kfPgenGlobalDosagePresent)) || ((!(vrtype & 0x60)) && (!subsetting_required))) {
        {
            const uint32_t need_unphased_het_ct = is_minimac3_r2 && VrtypeHphase(vrtype);
            PglErr reterr;
            if(!(subsetting_required && need_unphased_het_ct)) {
                reterr = GetBasicGenotypeCounts(sample_include, sample_include_interleaved_vec, sample_include_cumulative_popcounts, sample_ct, vidx,
                                                pgrp, need_unphased_het_ct ? (&unphased_het_ct) : nullptr, genocounts);
            } else {
                reterr = GetPhasedBiallelicGenotypeSubsetCounts(sample_include, sample_include_interleaved_vec, sample_ct, vidx, pgrp,
                                                                &unphased_het_ct, genocounts);
            }
            if(unlikely(reterr)) {
                return reterr;
            }
    ...
    if(!(vrtype & 0x60)) {
        if(is_minimac3_r2 && VrtypeHphase(vrtype)) {
            ...
            reterr = GetUnphasedBiallelicHetCt(subsetting_required ? sample_include : nullptr, raw_genovec, fread_ptr, fread_end, genocounts[1], pgrp,
                                               &unphased_het_ct);
            if(unlikely(reterr)) {
                return reterr;
            }
...
PglErr PgrGetDCounts(const uintptr_t *__restrict sample_include, const uintptr_t *__restrict sample_include_interleaved_vec,
                     PgrSampleSubsetIndex pssi, uint32_t sample_ct, uint32_t vidx, uint32_t is_minimac3_r2, PgenReader *pgr_ptr, double *imp_r2_ptr,
                     STD_ARRAY_REF(uint32_t, 4) genocounts, uint64_t *__restrict all_dosages) {
    if(!sample_ct) {
        STD_ARRAY_REF_FILL0(4, genocounts);
        all_dosages[0] = 0;
        all_dosages[1] = 0;
    ...
    PgenReaderMain *pgrp = GetPgrp(pgr_ptr);
    assert(vidx < pgrp->fi.raw_variant_ct);
    return GetBasicGenotypeCountsAndDosage16s(sample_include, sample_include_interleaved_vec, GetSicp(pssi), sample_ct, vidx, is_minimac3_r2, pgrp,
                                              imp_r2_ptr, genocounts, all_dosages);
}
...
PglErr CountAllAux1a(const unsigned char *fread_end, const uintptr_t *__restrict sample_include, const uintptr_t *__restrict raw_genoarr,
                     uint32_t aux1a_mode, uint32_t raw_sample_ct, uint32_t allele_ct, uint32_t raw_01_ct, const unsigned char **fread_pp,
                     uint64_t *__restrict one_cts, uint32_t *__restrict deltalist_workspace) {
    if(aux1a_mode == 15) {
        return kPglRetSuccess;
    }
    if(!sample_include) {
...
void CountAllAux1bDense(const void *__restrict patch_10_fvals, uint32_t allele_ct, uint32_t rare10_ct, uint64_t *__restrict one_cts_offset1,
                        uint64_t *__restrict two_cts_offset1) {
...
PglErr CountAllAux1b(const unsigned char *fread_end, const uintptr_t *__restrict sample_include, const uintptr_t *__restrict raw_genoarr,
                     uint32_t aux1b_mode, uint32_t raw_sample_ct, uint32_t allele_ct, uint32_t raw_10_ct, const unsigned char **fread_pp,
                     uint64_t *__restrict one_cts, uint64_t *__restrict two_cts, uint32_t *__restrict deltalist_workspace) {
    if(aux1b_mode == 15) {
        return kPglRetSuccess;
    }
...
PglErr GetMultiallelicCountsAndDosage16s(const uintptr_t *__restrict sample_include, const uintptr_t *__restrict sample_include_interleaved_vec,
                                         uint32_t sample_ct, uint32_t vidx, uint32_t allele_ct, __maybe_unused uint32_t is_minimac3_r2,
                                         PgenReaderMain *pgrp, double *__restrict imp_r2_ptr, uint32_t *__restrict het_ctp,
                                         STD_ARRAY_REF(uint32_t, 4) genocounts, uint64_t *all_dosages) {
    ...
    if(VrtypeMultiallelicHc(vrtype)) {
        ...
        uint32_t *deltalist_workspace = pgrp->workspace_difflist_sample_ids;
        reterr = CountAllAux1a(fread_end, sample_include, raw_genovec, aux1a_mode, raw_sample_ct, allele_ct, raw_het_ct, &fread_ptr, one_cts,
                               deltalist_workspace);
        if(unlikely(reterr)) {
            return reterr;
        }
        const unsigned char *aux1b_start = fread_ptr;
        reterr = CountAllAux1b(fread_end, sample_include, raw_genovec, aux1b_mode, raw_sample_ct, allele_ct, raw_10_ct, &fread_ptr, one_cts, two_cts,
                               deltalist_workspace);
        if(unlikely(reterr)) {
            return reterr;
        }
    ...
    if(raw_het_ct_needed) {
        if(!all_hets) {
            ...
            reterr =
                GetPhasepresentAndSkipPhaseinfo(fread_end, all_hets, raw_sample_ct, raw_het_ct, &fread_ptr, raw_phasepresent, &extra_phased_het_ct);
            if(unlikely(reterr)) {
                return reterr;
            }
            if(sample_include) {
...
PglErr PgrGetMDCounts(const uintptr_t *__restrict sample_include, const uintptr_t *__restrict sample_include_interleaved_vec,
                      PgrSampleSubsetIndex pssi, uint32_t sample_ct, uint32_t vidx, uint32_t is_minimac3_r2, PgenReader *pgr_ptr,
                      double *__restrict imp_r2_ptr, uint32_t *__restrict het_ctp, STD_ARRAY_REF(uint32_t, 4) genocounts,
                      uint64_t *__restrict all_dosages) {
    PgenReaderMain *pgrp = GetPgrp(pgr_ptr);
    assert(vidx < pgrp->fi.raw_variant_ct);
    const uintptr_t *allele_idx_offsets = pgrp->fi.allele_idx_offsets;
    ...
    if((allele_ct == 2) || (!(vrtype & 0x68))) {
        PglErr reterr = GetBasicGenotypeCountsAndDosage16s(sample_include, sample_include_interleaved_vec, sample_include_cumulative_popcounts,
                                                           sample_ct, vidx, is_minimac3_r2, pgrp, imp_r2_ptr, genocounts, all_dosages);
        *het_ctp = genocounts[1];
        ZeroU64Arr(allele_ct - 2, &(all_dosages[2]));
        return reterr;
    }
    return GetMultiallelicCountsAndDosage16s(sample_include, sample_include_interleaved_vec, sample_ct, vidx, allele_ct, is_minimac3_r2, pgrp,
                                             imp_r2_ptr, het_ctp, genocounts, all_dosages);
}

PglErr PgrGetMD(const uintptr_t *__restrict sample_include, PgrSampleSubsetIndex pssi, uint32_t sample_ct, uint32_t vidx, PgenReader *pgr_ptr,
                PgenVariant *pgvp) {
    pgvp->patch_01_ct = 0;
    pgvp->patch_10_ct = 0;
    pgvp->dosage_ct = 0;
    ...
    const uint32_t vrtype = GetPgfiVrtype(&(pgrp->fi), vidx);
    if((allele_ct == 2) || (!(vrtype & 0x68))) {
        return IMPLPgrGetD(sample_include, sample_include_cumulative_popcounts, sample_ct, vidx, pgrp, pgvp->genovec, pgvp->dosage_present,
                           pgvp->dosage_main, &(pgvp->dosage_ct));
    }
    const unsigned char *fread_ptr;
    const unsigned char *fread_end;
    uintptr_t *all_hets = VrtypeHphase(vrtype) ? pgrp->workspace_all_hets : nullptr;
...
PglErr IMPLPgrGetDp(const uintptr_t *__restrict sample_include, const uint32_t *__restrict sample_include_cumulative_popcounts, uint32_t sample_ct,
                    uint32_t vidx, PgenReaderMain *pgrp, PgenVariant *pgvp) {
    assert(vidx < pgrp->fi.raw_variant_ct);
    if(!sample_ct) {
        pgvp->phasepresent_ct = 0;
        pgvp->dosage_ct = 0;
    ...
    PglErr reterr = ReadGenovecHphaseSubsetUnsafe(sample_include, sample_include_cumulative_popcounts, sample_ct, vidx, pgrp,
                                                  dosage_is_present ? (&fread_ptr) : nullptr, dosage_is_present ? (&fread_end) : nullptr,
                                                  pgvp->genovec, pgvp->phasepresent, pgvp->phaseinfo, &(pgvp->phasepresent_ct));
    ...
    const uintptr_t *allele_idx_offsets = pgrp->fi.allele_idx_offsets;
    const uint32_t allele_ct = allele_idx_offsets ? (allele_idx_offsets[vidx + 1] - allele_idx_offsets[vidx]) : 2;
    return ParseDosage16(fread_ptr, fread_end, sample_include, sample_ct, vidx, allele_ct, pgrp, &(pgvp->dosage_ct), pgvp->dphase_present,
                         pgvp->dphase_delta, &(pgvp->dphase_ct), pgvp->dosage_present, pgvp->dosage_main);
}

PglErr PgrGetInv1Dp(const uintptr_t *__restrict sample_include, PgrSampleSubsetIndex pssi, uint32_t sample_ct, uint32_t vidx, AlleleCode allele_idx,
                    PgenReader *pgr_ptr, PgenVariant *pgvp) {
    PgenReaderMain *pgrp = GetPgrp(pgr_ptr);
    const uint32_t *sample_include_cumulative_popcounts = GetSicp(pssi);
    const uintptr_t *allele_idx_offsets = pgrp->fi.allele_idx_offsets;
    const uint32_t allele_ct = allele_idx_offsets ? (allele_idx_offsets[vidx + 1] - allele_idx_offsets[vidx]) : 2;
    ...
    if(!VrtypeDosage(vrtype)) {
        pgvp->dosage_ct = 0;
        pgvp->dphase_ct = 0;
        return IMPLPgrGetInv1P(sample_include, sample_include_cumulative_popcounts, sample_ct, vidx, allele_idx, pgrp, pgvp->genovec,
                               pgvp->phasepresent, pgvp->phaseinfo, &(pgvp->phasepresent_ct));
    }
...
PglErr PgrGetMDp(const uintptr_t *__restrict sample_include, PgrSampleSubsetIndex pssi, uint32_t sample_ct, uint32_t vidx, PgenReader *pgr_ptr,
                 PgenVariant *pgvp) {
    PgenReaderMain *pgrp = GetPgrp(pgr_ptr);
    const uint32_t *sample_include_cumulative_popcounts = GetSicp(pssi);
    pgvp->patch_01_ct = 0;
...
PglErr PgrGetRaw(uint32_t vidx, PgenGlobalFlags read_gflags, PgenReader *pgr_ptr, uintptr_t **loadbuf_iter_ptr, unsigned char *loaded_vrtype_ptr) {
    ...
    if(multiallelic_hc_present) {
        ...
        if(aux1a_mode != 15) {
            ...
            reterr =
                ExportAux1a(fread_end, genovec, aux1a_mode, raw_sample_ct, allele_ct, het_ct, &fread_ptr, patch_01_set, patch_01_vals, &rare01_ct);
            if(unlikely(reterr)) {
                return reterr;
            }
        ...
        if(aux1b_mode != 15) {
            ...
            reterr =
                ExportAux1b(fread_end, genovec, aux1b_mode, raw_sample_ct, allele_ct, raw_10_ct, &fread_ptr, patch_10_set, patch_10_vals, &rare10_ct);
            if(unlikely(reterr)) {
                return reterr;
            }
    ...
    *loadbuf_iter_ptr = loadbuf_iter;
    return ParseDosage16(fread_ptr, fread_end, nullptr, raw_sample_ct, vidx, allele_ct, pgrp, nullptr, dphase_present, dphase_delta, nullptr,
                         dosage_present, dosage_main);
}

// Currently assumes no phase or multiallelic hardcalls.
// tried to have more custom code, turned out to not be worth it
PglErr ReadMissingness(const uintptr_t *__restrict sample_include, const uint32_t *__restrict sample_include_cumulative_popcounts, uint32_t sample_ct,
                       uint32_t vidx, PgenReaderMain *pgrp, const unsigned char **fread_pp, const unsigned char **fread_endp,
                       uintptr_t *__restrict missingness, uintptr_t *__restrict hets, uintptr_t *__restrict genovec_buf) {
    const unsigned char *fread_ptr;
    const unsigned char *fread_end;
    PglErr reterr =
        ReadGenovecSubsetUnsafe(sample_include, sample_include_cumulative_popcounts, sample_ct, vidx, pgrp, &fread_ptr, &fread_end, genovec_buf);
...
PglErr PgrGetMissingness(const uintptr_t *__restrict sample_include, PgrSampleSubsetIndex pssi, uint32_t sample_ct, uint32_t vidx,
                         PgenReader *pgr_ptr, uintptr_t *__restrict missingness, uintptr_t *__restrict genovec_buf) {
    if(!sample_ct) {
        return kPglRetSuccess;
    }
    PgenReaderMain *pgrp = GetPgrp(pgr_ptr);
...
PglErr PgrGetMissingnessD(const uintptr_t *__restrict sample_include, PgrSampleSubsetIndex pssi, uint32_t sample_ct, uint32_t vidx,
                          PgenReader *pgr_ptr, uintptr_t *__restrict missingness_hc, uintptr_t *__restrict missingness_dosage,
                          uintptr_t *__restrict hets, uintptr_t *__restrict genovec_buf) {
    if(!sample_ct) {
        return kPglRetSuccess;
    }
    PgenReaderMain *pgrp = GetPgrp(pgr_ptr);
    ...
    if(!need_to_skip_aux1or2) {
        PglErr reterr =
            ReadMissingness(sample_include, sample_include_cumulative_popcounts, sample_ct, vidx, pgrp, dosage_is_relevant ? (&fread_ptr) : nullptr,
                            dosage_is_relevant ? (&fread_end) : nullptr, missingness_base, hets, genovec_buf);
        ...
        if(VrtypeHphase(vrtype) || hets) {
            ...
            if(VrtypeMultiallelicHc(vrtype)) {
                ...
                uint32_t *deltalist_workspace = pgrp->workspace_difflist_sample_ids;
                uint32_t aux1b_het_present;
                reterr = GetAux1bHets(fread_end, genovec_buf, aux1b_mode, raw_sample_ct, allele_ct, raw_10_ct, &fread_ptr, aux1b_hets,
                                      &aux1b_het_present, deltalist_workspace);
                if(unlikely(reterr)) {
                    return reterr;
                }
...
BoolErr ValidateDifflistHeader(const unsigned char *fread_end, uint32_t sample_ct, const unsigned char **fread_pp, uintptr_t *raregeno_buf,
                               const unsigned char **difflist_group_info_ptr, uint32_t *difflist_len_ptr) {
...
BoolErr ValidateAndApplyDifflist(const unsigned char *fread_end, uint32_t common2_code, const unsigned char **fread_pp, PgenReaderMain *pgrp,
                                 uintptr_t *__restrict genoarr) {
    // Side effects: uses pgr.workspace_raregeno_tmp_loadbuf.
...
BoolErr ValidateGeno(const unsigned char *fread_end, uint32_t vidx, PgenReaderMain *pgrp, const unsigned char **fread_pp, uintptr_t *genovec,
                     char *errstr_buf) {
    const uint32_t vrtype = pgrp->fi.vrtypes[vidx];
    const uint32_t sample_ct = pgrp->fi.raw_sample_ct;
    if(VrtypeLdCompressed(vrtype)) {
...
BoolErr ValidateAndCountDeltalist(const unsigned char *fread_end, uint32_t sample_ct, const unsigned char **fread_pp, uint32_t *__restrict deltalist,
                                  uint32_t *deltalist_len_ptr) {
...
BoolErr ValidateMultiallelicHc(const unsigned char *fread_end, const uintptr_t *__restrict raw_genovec, uint32_t vidx, uint32_t allele_ct,
                               PgenReaderMain *pgrp, const unsigned char **fread_pp, uint32_t *__restrict het_ctp, char *__restrict errstr_buf) {
    if(unlikely(allele_ct <= 2)) {
        snprintf(errstr_buf, kPglErrstrBufBlen,
                 "Error: Multiallelic hardcall track present for (0-based) variant #%u, but it apparently has only %u allele%s.\n", vidx, allele_ct,
                 (allele_ct == 1) ? "" : "s");
        return 1;
    }
    const uint32_t sample_ct = pgrp->fi.raw_sample_ct;
    const uint32_t aux1_first_byte = **fread_pp;
    *fread_pp += 1;
    if(unlikely(aux1_first_byte && (aux1_first_byte != 1) && (aux1_first_byte != 15) && (aux1_first_byte != 16) && (aux1_first_byte != 17) &&
                (aux1_first_byte != 31) && (aux1_first_byte != 240) && (aux1_first_byte != 241))) {
        snprintf(errstr_buf, kPglErrstrBufBlen,
                 "Error: Invalid multiallelic hardcall track mode byte (%u; must be in {0, 1, 15, 16, 17, 31, 240, 241}) in (0-based) variant #%u.\n",
    ...
    if(aux1a_mode != 15) {
        if(unlikely(!raw_01_ct)) {
            snprintf(errstr_buf, kPglErrstrBufBlen,
                     "Error: Multiallelic het-ref hardcall track present for (0-based) variant #%u, but no het-ref calls exist.\n", vidx);
            return 1;
        }
        uint32_t rare01_ct;
        if(!aux1a_mode) {
            ...
            *fread_pp += subset_byte_ct;
            ...
            if(raw_01_ct_mod8) {
                if(unlikely((*fread_pp)[-1] >> raw_01_ct_mod8)) {
                    snprintf(errstr_buf, kPglErrstrBufBlen,
                             "Error: Multiallelic het-ref hardcall bitarray-subset for (0-based) variant #%u has nonzero trailing bits.\n", vidx);
                    return 1;
                }
            }
        } else {
            ...
            for(uint32_t uii = 0; uii != rare01_ct; ++uii) {
                if(unlikely(GetNyparrEntry(raw_genovec, deltalist_workspace[uii]) != 1)) {
                    snprintf(errstr_buf, kPglErrstrBufBlen,
                             "Error: Invalid multiallelic het-ref hardcall deltalist-subset for (0-based) variant #%u (an index doesn't correspond "
                             "to a het-ref call).\n",
                             vidx);
                    return 1;
                }
        ...
        if(allele_ct < 5) {
            ...
            if(allele_ct == 4) {
                ...
                if(PtrAddCk(fread_end, fvals_byte_ct, fread_pp)) {
                    snprintf(errstr_buf, kPglErrstrBufBlen,
                             "Error: Invalid multiallelic het-ref hardcall track for (0-based) variant #%u (shorter than expected).\n", vidx);
                    return 1;
                }
                ...
                if(rare01_ct_mod8) {
                    if(unlikely((*fread_pp)[-1] >> rare01_ct_mod8)) {
                        snprintf(errstr_buf, kPglErrstrBufBlen,
                                 "Error: Invalid multiallelic het-ref hardcall track for (0-based) variant #%u (nonzero trailing bits).\n", vidx);
                        return 1;
                    }
                }
            }
                            ...
                            for(; widx != fullword_ct; ++widx) {
                                ...
                                if(unlikely(cur_word & (cur_word >> 1) & kMask5555)) {
                                    snprintf(
                                        ...
                                        "Error: Invalid multiallelic het-ref hardcall track for (0-based) variant #%u (out-of-range allele code).\n",
                                        vidx);
                                    return 1;
                                }
                            }
    ...
    if(aux1b_mode != 15) {
        if(unlikely(!raw_10_ct)) {
            snprintf(errstr_buf, kPglErrstrBufBlen,
                     "Error: Multiallelic altxy hardcall track present for (0-based) variant #%u, but no altxy calls exist.\n", vidx);
            return 1;
        }
        uint32_t rare10_ct;
        if(!aux1b_mode) {
            ...
            *fread_pp += subset_byte_ct;
            ...
            if(raw_10_ct_mod8) {
                if(unlikely((*fread_pp)[-1] >> raw_10_ct_mod8)) {
                    snprintf(errstr_buf, kPglErrstrBufBlen,
                             "Error: Multiallelic altxy hardcall bitarray-subset for (0-based) variant #%u has nonzero trailing bits.\n", vidx);
                    return 1;
                }
            }
        } else {
            ...
            for(uint32_t uii = 0; uii != rare10_ct; ++uii) {
                if(unlikely(GetNyparrEntry(raw_genovec, deltalist_workspace[uii]) != 2)) {
                    snprintf(errstr_buf, kPglErrstrBufBlen,
                             "Error: Invalid multiallelic altxy hardcall deltalist-subset for (0-based) variant #%u (an index doesn't correspond to "
                             "an altxy call).\n",
                             vidx);
                    return 1;
                }
        ...
        if(allele_ct < 6) {
            if(allele_ct == 3) {
                ...
                if(PtrAddCk(fread_end, fvals_byte_ct, fread_pp)) {
                    snprintf(errstr_buf, kPglErrstrBufBlen,
                             "Error: Invalid multiallelic altxy hardcall track for (0-based) variant #%u (shorter than expected).\n", vidx);
                    return 1;
                }
                ...
                if(rare10_ct_mod8) {
                    if(unlikely((*fread_pp)[-1] >> rare10_ct_mod8)) {
                        snprintf(errstr_buf, kPglErrstrBufBlen,
                                 "Error: Invalid multiallelic altxy hardcall track for (0-based) variant #%u (nonzero trailing bits).\n", vidx);
                        return 1;
                    }
                }
                ...
                if(nybble_cts[0]) {
                    snprintf(errstr_buf, kPglErrstrBufBlen,
                             "Error: Invalid multiallelic altxy hardcall track for (0-based) variant #%u (out-of-range allele code pair).\n", vidx);
                    return 1;
                }
            ...
            if(allele_ct < 18) {
                ...
                for(uint32_t uii = 0; uii != rare10_ct; ++uii) {
                    ...
                    if(unlikely((!hi_code) || (hi_code > max_code) || (lo_code > hi_code))) {
                        snprintf(errstr_buf, kPglErrstrBufBlen,
                                 "Error: Invalid multiallelic altxy hardcall track for (0-based) variant #%u (out-of-range or misordered allele code "
                                 "pair).\n",
                                 vidx);
                        return 1;
                    }
...
BoolErr ValidateHphase(const unsigned char *fread_end, uint32_t vidx, uint32_t het_ct, const unsigned char **fread_pp, char *errstr_buf) {
    if(unlikely(!het_ct)) {
        snprintf(errstr_buf, kPglErrstrBufBlen,
                 "Error: Hardcall phase track present for (0-based) variant #%u, but there were no heterozygous calls.\n", vidx);
        return 1;
    }
    const uint32_t aux2_first_part_byte_ct = 1 + (het_ct / CHAR_BIT);
...
#error "Unaligned accesses in ValidateDosage16()."
        ...
        for(uint32_t sample_idx = 0; sample_idx != sample_ct; ++sample_idx) {
            ...
            if(unlikely(cur_dosage_val_p1 > 32769)) {
                snprintf(errstr_buf, kPglErrstrBufBlen,
                         "Error: Invalid unconditional dosage track for (0-based) variant #%u (dosage is greater than 2).\n", vidx);
                return kPglRetMalformedInput;
            }
        }
        if(vrtype & 0x80) {
...
PglErr PgrValidate(PgenReader *pgr_ptr, uintptr_t *genovec_buf, char *errstr_buf) {
    ...
    for(uint32_t vidx = 0; vidx < variant_ct; vidx += kPglVblockSize) {
        if(unlikely(VrtypeLdCompressed(vrtypes[vidx]))) {
            snprintf(errstr_buf, kPglErrstrBufBlen,
                     "Error: (0-based) variant #%u is LD-compressed; this is prohibited when the variant index is a multiple of 65536.\n", vidx);
            return kPglRetMalformedInput;
        }
    }
    ...
    if(unlikely(expected_fsize != fsize)) {
        snprintf(errstr_buf, kPglErrstrBufBlen,
                 "Error: .pgen header indicates that file size should be %" PRIu64 " bytes, but actual file size is %" PRIu64 " bytes.\n",
    ...
    for(uint32_t vidx = 0; vidx != variant_ct; ++vidx) {
        ...
        if(vrtype & 0xe0) {
            if(unlikely((vrtype & 0xe0) == 0x80)) {
                snprintf(errstr_buf, kPglErrstrBufBlen,
                         "Error: Invalid record type for (0-based) variant #%u (phased dosage bit set, but main dosage bits unset).\n", vidx);
                return kPglRetMalformedInput;
            }
        ...
        if(unlikely(fread_ptr != fread_end)) {
            ...
            snprintf(errstr_buf, kPglErrstrBufBlen,
                     "Error: Extra byte(s) in (0-based) variant record #%u. (record type = %u; expected length = %" PRIuPTR ", actual = %" PRIuPTR
                     ")\n",

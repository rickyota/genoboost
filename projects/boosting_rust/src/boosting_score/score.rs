...
//fn add_score(scores: &mut [f64], wgt: &Wgt, genotypes: &[u8], n: usize) {
// split for cov?
//fn add_score(scores: &mut [f64], wgt: &WgtBoost, genotypes: &[u8], n: usize, covs: &Covs) {
pub fn add_score(scores: &mut [f64], wgt: &WgtBoost, genot: &Genot, covs: &Covs) {
    //println!("wgt {:?}", wgt);
    match wgt.wgt().kind() {
        WgtKind::Snv(_, _, mi) => {
            //println!("mi {}", mi.unwrap());
            // This should never panic.
            //let genot = genot_single::get_genotypes_snv(genotypes, mi.unwrap(), n);
            ...
            match wgt.wgt().model().coef() {
                Coef::Binary((const_ti, alpha_ti)) => {
                    ...
                    for (score, val) in scores.iter_mut().zip(genot_mi.iter()) {
                        let score_add = match (val as f64) > threshold {
                            true => score_add_high,
                            false => score_add_low,
                        };
                        *score += score_add;
...
fn calculate_write_score(
    ...
    for (item_i, wgt) in wgts.wgts().iter().enumerate() {
        ...
        //println!("iter_next,use{},{}", iter_next, iter_use);
        if item_i == *item_next {
            //println!("Write iter {}", iter_next);
            //write
            let fout_iteration =
                super::io::fname_score_createdir(dout, *item_fname, nocov, is_nsnv);
            println!("Write iteration {}: {:?}", item_fname, fout_iteration);

            super::io::write_scores(&fout_iteration, &scores, ys, samples_id);
...
fn create_item_last_indexs_write(iters_write: &[usize], iter_until_item: &[usize]) -> Vec<usize> {
    ...
    for (item_i, nsnv_until_iter) in iter_until_item.iter().enumerate().rev() {
        //println!("item_i, nsnv_until_iter {},{}", item_i, nsnv_until_iter);
        //println!("nsnv_next {}", nsnv_next);
...
pub fn calculate_write_score_iterations(
    ...
    //println!("iter_until_item {:?}", iter_until_item);
    //println!("iterations_write {:?}", iterations_write);
    ...
    if item_last_indexs_write.len() == 0 {
        println!("No iterations to be written.");
        return;
    }
...
pub fn calculate_write_score_nsnvs(
    ...
    //println!("nsnvs_until_item {:?}", nsnvs_until_item);

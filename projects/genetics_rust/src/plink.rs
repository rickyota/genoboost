...
// TODO: what if fin.fam includes phe=9?
pub fn load_ys(fin: &Path, n: usize, use_samples: &[bool]) -> Vec<bool> {
    let len_n = n;
...
/// create hashmap of tuple of (chrom,pos,a1,a2) like samples -> ng
/// ref: io_genetics.cc:1955
// TODO: snvs_in: HaveSnvIndex
pub fn make_use_snvs(fin_snv: Option<&Path>, snvs_in: &Vec<SnvId>) -> (usize, Vec<bool>) {
    ...
    let mut sida_in_to_index = HashMap::with_capacity(m_in);

    for (si, s) in snvs_in.iter().enumerate() {
        sida_in_to_index.insert(s.sida(), si);
    }
    ...
    for snv in snvs_use.iter() {
        ...
        let sida = snv.sida();
        // FIXME: for None
        if let Some(v) = sida_in_to_index.get(sida) {
            use_snvs[*v] = true;
            m += 1;
        }

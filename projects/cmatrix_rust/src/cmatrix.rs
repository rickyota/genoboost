...
///
/// ex.  row=3, col=2, val=[0,1,2,1,2,3] (in 2bit)
/// ax0=3, ax1=2, ax2=2
/// ax1=0 shows dominant model, and ax1=1 shows recessive model
/// inner=[0b01111000,0,0,0,0, 0b001001,0,0,0,0]
///
///
///
...
impl CMatrix {
    ...
    #[inline]
    fn len_arg(row_n: usize, col_n: usize, digit_n: usize) -> usize {
    ...
    /// Vec is aligned
    pub fn new_empty(row_n: usize, col_n: usize, digit: usize) -> CMatrix {
        CMatrix::check_new(row_n, col_n, digit);
        // just for len()
        let m = CMatrix {
            inner: vec![],
            row_n,
    ...
    pub fn new(row: usize, col: usize, digit: usize, vec: Vec<u8>) -> CMatrix {
        ...
        let mut m = CMatrix::new_empty(row, col, digit);
    ...
    // -> panic
    pub fn new_vec_bool(row: usize, col: usize, vec: Vec<bool>) -> CMatrix {
        let digit = 1;
        CMatrix::check_new(row, col, digit);
...
mod tests {
    ...
    #[test]
    fn test_to_cmatrix_mut() {
        let mut m = setup1();
        let mref = m.to_cmatrix_mut();

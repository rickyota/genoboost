...
///
/// When accessing index, using iterator is efficient since no index boudnary check is run.
///
///
...
pub trait BaseCMatrix {
    ...
    // or in index_byte? -> no, index_byte works in any situaltion
    #[inline]
    ...
    fn to_cmatrix_ref(&self) -> CMatrixRef {
        CMatrixRef::new(self.inner(), self.row_n(), self.col_n(), self.digit_n())
    }

    fn to_cvec_ref(&self, row_i: usize) -> CVecRef {
        CVecRef::new(
    ...
    fn iter_row(&self) -> RowIter {
        RowIter::new(self.to_cmatrix_ref())
    }
...
pub trait BaseCMatrixMut: BaseCMatrix {
    ...
    // TODO: check val<4
    // use set() since rowi and coli is guranteed
    ...
    /*     // TODO: check val<4
    // TODO: improve: not beautiful to use set_col() in BaseCVec
